  ...
  collectPosts: function () {
    ...
    if (findResult.fulltextResultItems && findResult.fulltextResultItems.length > 0) {
      for (i = 0, length = findResult.fulltextResultItems.length; i < length; i++) {
        ...
        jsonSord.push('" , "sord": ');
        jsonSord.push(formatter.buildJson(findResult.fulltextResultItems[i].sord, me.docMasks[findResult.fulltextResultItems[i].sord.maskName], { dataAsString: ["KNOWLEDGE_TOPICS"] }));
        jsonSord.push(" }");
      ...
      for (i = 0, length = findResult.sords.length; i < length; i++) {
        ...
        jsonData.push(formatter.buildJson(findResult.sords[i], me.docMasks[findResult.sords[i].maskName], { dataAsString: ["KNOWLEDGE_TOPICS"] }));
      }
  ...
  collectPostFindResult: function () {
    ...
    if (!me.searchId || (me.searchId === -1)) {
      ...
      if (!me.idx || (me.idx === 0)) {
        ...
        findResult = ixConnect.ix().findFirstSords(findInfo, 0, me.sordZ); // init a new search
        findResult = ixConnect.ix().findNextSords(findResult.searchId, me.idx, me.max, me.sordZ); // return a resulsset different from the first one
      }
    } else {
      try {
