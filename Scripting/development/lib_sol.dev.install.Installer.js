...
sol.define("sol.dev.install.functions.ChangeRights", {
  ...
  execute: function () {
    ...
    me.config.install.changeRights.forEach(function (rightsEntry) {
      ...
      me.handler.log({ text: "Change rights: config=" + JSON.stringify(rightsEntry) });
      try {
  ...
  execute: function () {
    ...
    for (i = 0; i < me.workflowTemplateNames.length; i++) {
      try {
        ...
        exceptionString = sol.common.ExceptionUtils.parseException(ex);
        me.handler.log({ text: "Can't import workflow '" + workflowTemplateName + "' :" + exceptionString });
      }
    }
  }
});
...
sol.define("sol.dev.install.functions.InstallIxPlugins", {
  ...
  execute: function () {
    var me = this,
        i, ixPluginFile, inputStream, ixPluginFileName, exceptionString;
...
sol.define("sol.dev.install.functions.ShowAsConfigDialog", {
  ...
  updateAsConfig: function () {
    var me = this,
        asConfigObj, path, parentPath, name, objId;
...
sol.define("sol.dev.install.functions.ExecuteRegisteredFunctions", {
  ...
  execute: function () {
    ...
    if (me.config.install.executeRegisteredFunctions.reloadScripts) {
      ...
      me.handler.log({ text: "connProps=" + me.conn.connProperties });
  ...
  execute: function () {
    ...
    if (appsRootFolderId) {
      ...
      appFolders.forEach(function (folder) {
        manifest = me.getManifestObj(folder.id);
        if (manifest) {
          me.sendHttpRequest("Install ELOapp '" + manifest.id + "'", "{{eloWfBaseUrl}}/apps/rest/cmd/app/deploy/?namespace=" + manifest.namespace + "&appId=" + manifest.id, { method: "post" });
        } else {
...
sol.define("sol.dev.install.Installer", {
  ...
  initialize: function (params) {
    ...
    me.register("transport", "sol.dev.install.functions.RenameWorkflowTemplates");
    ...
    me.register("transport", "sol.dev.install.functions.ShowAsConfigDialog");
