...
sol.define("sol.common_monitoring.ix.services.SearchTimedEvents", {
  ...
  messages: {
    missingSearchTemplate: "Could not determine search template",
    missingResult: "could not get search result"
  },
  ...
  getIdsFromResult: function (result) {
    return ((result || {}).sords || [])
      .map(function (obj) {
        return obj
          ? sol.common.ObjectUtils.type(obj, "string")
            ? obj
            : obj.id
              ? obj.id
              : obj.objId
...
function onEnterNode(clInfo, userId, wfDiagram, nodeId) {
  ...
  sol.create("sol.common_monitoring.ix.services.SearchTimedEvents", params).process();
}
