...
sol.define("sol.pubsec.ix.Routines", {
  ...
  retrieveRoutines: function () {
    var me = this,
        mapcontent, entriesIterator, entry, routineObjId, routineSord, routines, blobStream;

    if (!me.multipleRoutinesSupported || me.isTemplate) {
      routineObjId = me.getExternalRoutineObjId();

      if (!routineObjId || (routineObjId.trim() == "")) {
        routineSord = me.getProcessSord();
      } else {
        if (sol.common.AclUtils.hasEffectiveRights(me.getProcessSord(), { rights: { r: true } })) {
          routineSord = ixConnectAdmin.ix().checkoutSord(routineObjId, SordC.mbAllIndex, LockC.NO);
        }
      }

      routines = (routineSord && routineSord.desc && !routineSord.desc.trim().isEmpty()) ? [JSON.parse(routineSord.desc)] : null;

      if (routines && routineSord) {
        routines[0].name = routineSord.name;
        routines[0].status = sol.common.SordUtils.getObjKeyValue(routineSord, me.config.fields.routineStatus);
      }
    } else {
  ...
  saveExternalRoutine: function (name, routineTpl) {
    ...
    if (me.isTemplate) {
      ...
      routineSord = me.getProcessSord();
    } else if ((!routineObjId || routineObjId.trim().isEmpty()) && sol.common.AclUtils.hasEffectiveRights(me.getProcessSord(), { rights: { r: true, w: true } })) {
