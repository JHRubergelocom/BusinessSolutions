...
sol.define("sol.common.as.functions.Export", {
  ...
  /**
   * @cfg {Boolean} [includeFirstDocChildData=false]
   * Include the data of the first document child
   */
...
sol.define("sol.common.as.ExporterBase", {
  ...
  initialize: function (config) {
    ...
    me.includeFirstDocChildData = config.includeFirstDocChildData || false;
  },
  ...
  export: function () {
    var me = this,
        ...
        i, payloadId, payloadName, numberOfPayloads, sord, tplSord, firstDocChildSord;

    try {
      sord = ixConnect.ix().checkoutSord(me.objId, EditInfoC.mbSord, LockC.NO).sord;
      if (sord.lockName && sord.lockName.length() > 0) {
        log.warn("Export is skipped because the object (id: " + me.objId + ") is locked by " + sord.lockName);
        return { passOn: false };
      }
      tplSord = sol.common.SordUtils.getTemplateSord(sord);

      if (sol.common.SordUtils.isFolder(sord)) {
        tplSord.sord = tplSord.sord || {};
        tplSord.sord.firstDocChild = {};
        firstDocChildSord = sol.common.RepoUtils.getFirstChild({ parentId: sord.id, includeFolders: false });
        if (firstDocChildSord) {
          tplSord.sord.firstDocChild.id = firstDocChildSord.id + "";
          tplSord.sord.firstDocChild.guid = firstDocChildSord.guid + "";
        }
      }

      me.logger.info("content=" + JSON.stringify(tplSord));
  ...
  getTemplateExtension: function () {
    ...
    try {
      ...
      throw "Can't find template: objId=" + me.tplObjId;
    }
