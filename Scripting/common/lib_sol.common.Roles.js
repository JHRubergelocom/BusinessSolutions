...
sol.define("sol.common.Roles", {
  ...
  getUsers2: function (role, sord, originalConfig) {
    ...
    me.logger.exit("getUsers2", userEntries);
  ...
  getSupervisor: function (userDef, userName) {
    ...
    userDef.supervisorLevel = (typeof userDef.supervisorLevel == "undefined") ? 0 : userDef.supervisorLevel;
    supervisors = sol.common.UserUtils.getSupervisorHierarchy(userName);
    if (!supervisors || (supervisors.length <= userDef.supervisorLevel)) {
      me.logger.debug(["Can't find supervisor: userConfig={0}, userName={1}, supervisors={2}", JSON.stringify(userDef), userName, supervisors]);
      return;
    }
    supervisor = supervisors[userDef.supervisorLevel];
