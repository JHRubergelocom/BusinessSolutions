...
sol.define("sol.common.SordProvider", {
   ...
   */
  BASICSORDKEYS: ["ID", "GUID", "NAME", "DESC", "MASK", "XDATEISO", "IDATEISO"],
  ...
  sordKeysExtractor: function (result, sord, fields) {
    var me = this, field, i = fields.length, targetProp, ignorePropNames = me.ipn;
    while (i--) {
      field = fields[i];
  ...
  addConverterKey: function (converterConfig, type, key, prop) {
  ...
  addAsSourceTargetInstruction: function (instructions, source, target) {
    ...
    me.addConverterKey(instructions.converterConfig, source.type, source.key, target.prop);
  ...
  finalizeCollector: function (instructions) {
    ...
    if (onlyOneMbAndItsForAnId && (instructions.converterConfig && (!instructions.converterConfig.mapKeys && !instructions.converterConfig.formBlobs))) {
      ...
      instructions.sordZ = me.createSordZ(me.generateMbsFromOptions(instructions.mbs));
    }
  },
