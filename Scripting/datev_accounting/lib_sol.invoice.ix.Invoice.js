 ...
 *
 * Uses invoice configuration object from `/Administration/Business Solutions/sol.datev.accounting/Configuration/sol.invoice.config`
 *
...
sol.define("sol.invoice.ix.Invoice", {
  ...
  initialize: function (config) {
    ...
    me.sord = config.sord;
    me.config = sol.create("sol.common.Config", { compose: "/sol.datev.accounting/Configuration/sol.invoice.config" }).config;
  ...
  setDefaultValues: function () {
    ...
    try {
      ...
      me.logger.warn(["Could not set mask '{0}'", me.config.invoiceMaskName.value], ex);
    }
    // Set company name
    try {
      ...
      me.logger.warn(["Could not set company name by company code"], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not set company code by company name"], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not set business area name by business area code"], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not set vendor no"], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not set invoice type field '{0}'", me.config.fields.INVOICE_TYPE.value], ex);
    }
    ...
    try {
      if (!sol.common.SordUtils.getObjKeyValue(me.sord, me.config.fields.INVOICE_CURRENCY_CODE.value)) {
       ...
       me.logger.warn(["Could not set currency code field '{0}'", me.config.fields.INVOICE_CURRENCY_CODE.value], ex);
    }
    ...
    try {
      sol.common.ix.DynKwlUtils.fillMap(null, me.sord, me.config.fields.INVOICE_CURRENCY_CODE.value, me.config.kwlScripts.currency.value);
       ...
       me.logger.warn(["Could not fill map exhange rate via '{0}'", me.config.kwlScripts.currency.value], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not fill map termsOfPayment via '{0}'", me.config.kwlScripts.termsOfPayment.value], ex);
    }
    ...
    try {
      ...
      me.logger.warn(["Could not set charge codes"], ex);
    }
  ...
  correctCreditNoteKey: function (invoiceType) {
    var me = this,
        keyExists;
    if (invoiceType == "CR") {
      keyExists = me.localizedKwlKeyExists("/sol.datev.accounting/Configuration/sol.invoice.LocalizedKwls.config", "docType", "CR");
      if (!keyExists) {
