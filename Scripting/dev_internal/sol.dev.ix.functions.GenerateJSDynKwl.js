...
var logger = sol.create("sol.Logger", { scope: "sol.dev.ix.functions.GenerateJSDynKwl" });
/**
 * Generate JSDynKwls from templates.
 *
 ...
 *
 *     sol.common.IxUtils.execute('RF_sol_dev_function_GenerateJSDynKwl', {
 *       objId: "4711"
 *     });
 *
 ...
 *
 */
sol.define("sol.dev.ix.functions.GenerateJSDynKwl", {
  ...
  process: function () {
    ...
    if (!me.objId) {
      throw "sol.dev.ix.functions.GenerateJSDynKwl(): Object Id must not be empty";
    }
    ...
    me.logger.info(["generate jsdynkwl sord (objId={0}, name={1}): {2} -> {3}", me.objId]);
...
/**
 * @member sol.dev.ix.functions.GenerateJSDynKwl
 * @static
 * @inheritdoc sol.common.ix.FunctionBase#onEnterNode
 */
function onEnterNode(clInfo, userId, wFDiagram, nodeId) {
  logger.enter("onEnterNode_GenerateJSDynKwl", { flowId: wFDiagram.id, nodeId: nodeId });
  var params = sol.common.WfUtils.parseAndCheckParams(wFDiagram, nodeId),
      module;

  params.objId = wFDiagram.objId;
  module = sol.create("sol.dev.ix.functions.GenerateJSDynKwl", params);

  module.process();

  logger.exit("onEnterNode_GenerateJSDynKwl");
}
...
function onExitNode(clInfo, userId, wFDiagram, nodeId) {
  logger.enter("onExitNode_GenerateJSDynKwl", { flowId: wFDiagram.id, nodeId: nodeId });
  var params = sol.common.WfUtils.parseAndCheckParams(wFDiagram, nodeId),
      module;
  ...
  logger.exit("onExitNode_GenerateJSDynKwl");
}


/**
 * @member sol.dev.ix.functions.GenerateJSDynKwl
 * @method RF_sol_dev_function_GenerateJSDynKwl
 * @static
 * @inheritdoc sol.common.ix.FunctionBase#RF_FunctionName
 */
function RF_sol_dev_function_GenerateJSDynKwl(iXSEContext, args) {
  logger.enter("RF_sol_dev_function_GenerateJSDynKwl", args);
  var params = sol.common.ix.RfUtils.parseAndCheckParams(iXSEContext, arguments.callee.name, args, "objId"),
      module = sol.create("sol.dev.ix.functions.GenerateJSDynKwl", params);

  module.process();

  logger.exit("RF_sol_dev_function_GenerateJSDynKwl");
}
