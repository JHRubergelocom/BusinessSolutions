...
var logger = sol.create("sol.Logger", { scope: "sol.contract.ix.actions.TerminateContract" });
 ...
 */
sol.define("sol.contract.ix.actions.TerminateContract", {
  ...
  process: function () {
    ...
    wfName = sol.create("sol.common.Template", { source: contractConfig.workflows.terminateContract.workflowNameTemplate }).apply({ wfDate: new Date(), wfNumber: wfNumber });

    flowId = me.startWorkflow(contractObjId, contractConfig.workflows.terminateContract.workflowTemplateName, wfName);

    dialogTitle = sol.common.TranslateTerms.translate("sol.contract.client.ribbon.btnInitiateTermination");
...
/**
 * @member sol.contract.ix.actions.TerminateContract
 * @method RF_sol_contract_action_TerminateContract
 * @static
 * @inheritdoc sol.common.ix.FunctionBase#RF_FunctionName
 */
function RF_sol_contract_action_TerminateContract(ec, configAny) {
  logger.enter("RF_sol_contract_action_TerminateContract", configAny);
  var rfUtils = sol.common.ix.RfUtils,
      config = rfUtils.parseAndCheckParams(ec, arguments.callee.name, configAny, "objId"),
      terminateContract, result;

  config.ci = ec.ci;
  config.user = ec.user;

  terminateContract = sol.create("sol.contract.ix.actions.TerminateContract", config);
  result = terminateContract.execute();
  logger.exit("RF_sol_contract_action_TerminateContract", result);
  return result;
}
