...
var logger = sol.create("sol.Logger", { scope: "sol.contract.ix.actions.ApproveContract" });
...
sol.define("sol.contract.ix.actions.ApproveContract", {
  ...
  process: function () {
    ...
    wfName = sol.create("sol.common.Template", { source: contractConfig.workflows.approveContract.workflowNameTemplate }).apply({ wfDate: new Date(), wfNumber: wfNumber });

    flowId = me.startWorkflow(contractObjId, contractConfig.workflows.approveContract.workflowTemplateName, wfName);

    dialogTitle = sol.common.TranslateTerms.translate("sol.contract.client.ribbon.btnInitiateApproval");
...
function RF_sol_contract_action_ApproveContract(ec, configAny) {
  logger.enter("RF_sol_contract_action_ApproveContract", configAny);
  var rfUtils = sol.common.ix.RfUtils,
      config = rfUtils.parseAndCheckParams(ec, arguments.callee.name, configAny, "objId"),
      approveContract, result;

  config.ci = ec.ci;
  config.user = ec.user;

  approveContract = sol.create("sol.contract.ix.actions.ApproveContract", config);
  result = approveContract.execute();
  logger.exit("RF_sol_contract_action_ApproveContract", result);
  return result;
}
