 ...
 */
sol.define("sol.hr.ix.dynkwl.AuthorityIterator", {
  extend: "sol.hr.ix.dynkwl.PersonnelFileIterator",

  mixins: ["sol.hr.mixins.Configuration"],

  inject: {
    dynkwl: { config: "hr", prop: "entities.file.dynkwls.authority", template: true }, // {}
 ...
 * @static
 * @member sol.hr.ix.dynkwl.AuthorityIterator
 * @return {DynamicKeywordDataProvider}
 */
function getDataIterator() {
  var log = sol.create("sol.Logger", { scope: "sol.hr.ix.dynkwl.AuthorityIterator" }),
      iterator;
  try {
    log.info("DynamicKeywordList (");
    iterator = sol.create("sol.hr.ix.dynkwl.AuthorityIterator", {});
