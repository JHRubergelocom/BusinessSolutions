...
var logger = sol.create("sol.Logger", { scope: "sol.learning.ix.functions.generators.GenSessionShortDesc" });
 ...
 */
sol.define("sol.learning.ix.functions.generators.GenSessionShortDesc", {
  ...
  inject: {
    templateField: { config: "learning", prop: "entities.session.functions.generatesessionshortdescription.generatorConfig.generatorTemplateField", template: true }, // ""
    templateFolder: { config: "learning", prop: "entities.session.functions.generatesessionshortdescription.generatorConfig.templatesFolder", template: true } // ""
  },
  ...
  getIdentifierTemplateId: function () {
    var me = this;
    return me.getTemplateId("Session short description", me.templateField, me.templateFolder);
  },
...
function onEnterNode(_clInfo, _userId, wFDiagram, nodeId) {
  ...
  genProc = sol.create("sol.learning.ix.functions.generators.GenSessionShortDesc", params);
 ...
 * @method RF_sol_learning_function_generateSessionShortDesc
 * @static
 * @inheritdoc sol.common.ix.FunctionBase#RF_FunctionName
 */
function RF_sol_learning_function_generateSessionShortDesc(iXSEContext, args) {
  logger.enter("RF_sol_common_function_generateSessionReference", args);
  var params = sol.common.ix.RfUtils.parseAndCheckParams(iXSEContext, arguments.callee.name, args, "objId"),
      genProc = sol.create("sol.learning.ix.functions.generators.GenSessionShortDesc", params),
      result;
  result = genProc.process();
  logger.exit("RF_sol_learning_function_generateSessionShortDesc");
  return result;
}
