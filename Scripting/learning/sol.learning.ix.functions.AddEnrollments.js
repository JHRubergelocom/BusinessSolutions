...
sol.define("sol.learning.ix.functions.AddEnrollments", {
  ...
  adjustConfigForRow: function (config, row) {
    return config.metadataMapping  // set values from maptables in mapping
  ...
  rowToEnrollment: function (config, results, row) {
    var me = this;
    results.push(
      (me.createEnrollment(me.adjustConfigForRow(JSON.parse(config), row), row) || {}).data
    );
    return results;
  },
  ...
  process: function () {
    var me = this, createdEnrollments = [], user, course, curStatus;
    ...
    if (me.enrollRelatedCourses) {
      if (me.sord.mapKeys.COURSE_ENROLLMENT_CREATED_AUTO !== "1") {
        ...
        if ((curStatus = String(me.sord.objKeys.COURSE_ENROLLMENT_STATUS)) === "RUNNING") {
          me.manageEnrollment({ user: user, course: course, action: "started" });
        } else if (curStatus === "COMPLETED") {
          me.manageEnrollment({ user: user, course: course, action: "completed" });
        } else {
