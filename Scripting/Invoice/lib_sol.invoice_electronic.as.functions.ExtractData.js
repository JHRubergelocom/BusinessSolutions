...
sol.define("sol.invoice_electronic.as.functions.ExtractData", {
  ...
  process: function () {
    var me = this,
        ...
        i, postProcessingClassName, postProcessingClass, exceptionString;
    ...
    if (me.xmlFilePath) {
      ...
      try {
        ...
        me.logger.info(["Exception while invoice classification: {0}", exceptionString]);
      }
      ...
      try {
        me.logger.info(["xmlImporter.process(new File(xmlFile), me.objId) executed: sord.guid = '{0}' sord.name = '{1} xmlFile = '{2}'", sord.guid, sord.name, me.xmlFilePath]);
        xmlImporter.process(new File(me.xmlFilePath), me.objId);
      } catch (ex) {
        exceptionString = sol.common.ExceptionUtils.parseException(ex);
        me.logger.info(["Exception while extracting invoice data: {0}", exceptionString]);
      }
