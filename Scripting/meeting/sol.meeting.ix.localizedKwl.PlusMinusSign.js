 ...
 */
sol.define("sol.meeting.ix.localizedKwl.PlusMinusSign", {
  extend: "sol.meeting.ix.localizedKwl.Base",
  kwlName: "PlusMinusSign"
});

/**
 * Implements a localized keyword list for Reference Dates
 * @static
 * @member sol.meeting.ix.localizedKwl.PlusMinusSign
 * @returns {DynamicKeywordDataProvider}
 */
function getDataIterator() {
  var log = sol.create("sol.Logger", { scope: "sol.meeting.ix.localizedKwl.PlusMinusSign" }),
      iterator;

  try {
    log.info("LocalizedKeywordList (");

    iterator = sol.create("sol.meeting.ix.localizedKwl.PlusMinusSign", {});
