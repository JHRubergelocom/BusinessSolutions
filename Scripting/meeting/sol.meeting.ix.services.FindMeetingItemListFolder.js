...
/**
 * Retrieves Meeting Item List sord object.
 *
 ...
 */
sol.define("sol.meeting.ix.services.FindMeetingItemListFolder", {
  ...
  inject: {
    service: { config: "meeting", prop: "entities.meeting.services.GetMeetingItemListFolder", template: true },
 ...
 */
 function RF_sol_meeting_function_FindMeetingItemListFolder(iXSEContext, args) {
  var params, command, result;
  logger.enter("RF_sol_meeting_function_FindMeetingItemListFolder", args);

  params = sol.common.ix.RfUtils.parseAndCheckParams(iXSEContext, arguments.callee.name, args);

  command = sol.create("sol.meeting.ix.services.FindMeetingItemListFolder", params);

  result = command.process();
  logger.exit("RF_sol_meeting_function_FindMeetingItemListFolder");
