...
var logger = sol.create("sol.Logger", { scope: "sol.meeting.ix.services.FindMeetingParticipantListFolder" });
 ...
 */
sol.define("sol.meeting.ix.services.FindMeetingParticipantListFolder", {
  ...
  inject: {
    service: { config: "meeting", prop: "entities.meeting.services.GetMeetingParticipantListFolder", template: true },
 ...
 */
 function RF_sol_meeting_function_FindMeetingParticipantListFolder(iXSEContext, args) {
  var params, command, result;
  logger.enter("RF_sol_meeting_function_FindMeetingParticipantListFolder", args);

  params = sol.common.ix.RfUtils.parseAndCheckParams(iXSEContext, arguments.callee.name, args);

  command = sol.create("sol.meeting.ix.services.FindMeetingParticipantListFolder", params);

  result = command.process();
  logger.exit("RF_sol_meeting_function_FindMeetingParticipantListFolder");
