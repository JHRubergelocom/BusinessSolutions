...
describe("[dynkwl] sol.contact.ix.dynkwls.generators.NameCompany", function () {
  var originalTimeout, objGeneratorsNameCompanyId, config;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("GeneratorsNameCompany").then(function success(objGeneratorsNameCompanyId1) {
        objGeneratorsNameCompanyId = objGeneratorsNameCompanyId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.contact.ix.dynkwl.generators.NameCompany", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_productivity_service_ExecuteLib1", {
            className: "sol.contact.ix.dynkwl.generators.NameCompany",
            classConfig: {},
            method: "initialize",
            params: [config]
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contact.ix.dynkwl.generators.NameCompany", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_productivity_service_ExecuteDynKwl", {
            objId: objGeneratorsNameCompanyId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_productivity_service_ExecuteDynKwl", {
            objId: objGeneratorsNameCompanyId,
            dynKwl: "sol.contact.ix.dynkwl.generators.NameCompany"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
