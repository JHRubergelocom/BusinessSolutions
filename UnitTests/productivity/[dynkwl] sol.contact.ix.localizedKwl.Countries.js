...
describe("[dynkwl] sol.contact.ix.localizedKwl.Countries", function () {
  var originalTimeout, objCountriesId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Countries").then(function success(objCountriesId1) {
        objCountriesId = objCountriesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contact.ix.dynkwl.Countries", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_productivity_service_ExecuteDynKwl", {
            objId: objCountriesId,
            dynKwl: "sol.contact.ix.localizedKwl.Countries"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
