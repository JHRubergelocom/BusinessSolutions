...
describe("[dynkwl] sol.privacy.ix.dynkwls.Departments", function () {
  var originalTimeout, objDepartmentsId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Departments").then(function success(objDepartmentsId1) {
        objDepartmentsId = objDepartmentsId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.privacy.ix.dynkwl.Departments", function () {
      it("get 'Departments' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            objId: "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/privacy/Configuration/kwl.config"
          }).then(function success(configResult) {
            kwlConfig = configResult.config["Departments"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
