...
describe("[dynkwl] sol.meeting.ix.dynkwls.ParticipantGroupMember", function () {
  var originalTimeout, objParticipantGroupMemberId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ParticipantGroupMember").then(function success(objParticipantGroupMemberId1) {
        objParticipantGroupMemberId = objParticipantGroupMemberId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.ParticipantGroupMember", function () {
      it("get 'ParticipantGroupMember' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["ParticipantGroupMember"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objParticipantGroupMemberId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
