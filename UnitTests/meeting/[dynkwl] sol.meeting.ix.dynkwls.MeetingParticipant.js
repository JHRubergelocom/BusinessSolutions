...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingParticipant", function () {
  var originalTimeout, objMeetingParticipantId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MeetingParticipant").then(function success(objMeetingParticipantId1) {
        objMeetingParticipantId = objMeetingParticipantId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.MeetingParticipant", function () {
      it("get 'meetingParticipant' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["meetingParticipant"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
