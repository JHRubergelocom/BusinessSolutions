...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingItemPools", function () {
  var originalTimeout, objMeetingItemPoolsId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MeetingItemPools").then(function success(objMeetingItemPoolsId1) {
        objMeetingItemPoolsId = objMeetingItemPoolsId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.MeetingItemPools", function () {
      it("get 'meetingItemPools' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["meetingItemPools"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
