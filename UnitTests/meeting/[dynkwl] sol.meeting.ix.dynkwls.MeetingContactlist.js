...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingContactlist", function () {
  var originalTimeout, objParticipantContactlistId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MeetingContactlist").then(function success(objParticipantContactlistId1) {
        objParticipantContactlistId = objParticipantContactlistId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.MeetingContactlist", function () {
      it("get 'meetingContactlist' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["meetingContactlist"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objParticipantContactlistId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
