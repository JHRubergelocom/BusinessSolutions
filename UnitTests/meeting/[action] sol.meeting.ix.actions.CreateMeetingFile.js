...
describe("[action] sol.meeting.ix.actions.CreateMeetingFile", function () {
  var objTempId, meetingBoardTypes, configTypes, configAction, wfInfo, succNodes, succNodesIds,
      objIdMB, objIdM, originalTimeout, interval, meetingItemTypes;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.CreateMeetingFile", null, null).then(function success(objTempId1) {
  ...
  describe("test create meeting file", function () {
  ...
  describe("create meeting item 'premium'", function () {
    ...
    it("fill meeting item", function (done) {
      expect(function () {
        test.Utils.getSord(wfInfo.objId).then(function success(sordMI) {
          test.Utils.updateKeywording(sordMI, {
            ...
            test.Utils.updateSord(sordMI, [{ key: "desc", value: "Unittest Thema" }]).then(function success2(updateSordResult) {
              done();
            }, function error(err) {
              fail(err);
              console.error(err);
  ...
  describe("test finish create meeting file", function () {
    it("check precondition meeting file", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_premium_service_MeetingFilePrecondition", { targetId: objIdM }).then(function success(checkResult) {
    ...
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          ...
          $wf: {
            template: {
              name: "sol.meeting_premium.CreateMeetingFile"
            },
            name: "{{translate 'sol.meeting_premium.createMeetingFile.prefix'}}"
          },
          $events: [
            {
              id: "DIALOG",
            ...
            },
            {
              id: "FEEDBACK",
              onWfStatus: "CREATE",
              message: "{{translate 'sol.meeting_premium.client.meetingFile.feedback.create'}}"
            }
          ]
        };
        wfInfo = {};
    ...
    it("Configure meeting file", function (done) {
      expect(function () {
        test.Utils.updateMapData(objIdM, {
          MEETING_FILE_DOCUMENTNAME: "Test Sitzungsmappe",
          MEETING_FILE_VERSION_COMMENT: "Neue Version"
  ...
  describe("test cancel create meeting file", function () {
    it("check precondition meeting file", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_premium_service_MeetingFilePrecondition", { targetId: objIdM }).then(function success(checkResult) {
          expect(checkResult.valid).toEqual(true);
