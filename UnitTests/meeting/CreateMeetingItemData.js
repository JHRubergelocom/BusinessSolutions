...
describe("Create Meetingitem data", function () {
  var configTypes, configAction, meetingItemDataConfig,
      originalTimeout, RFs, RFfunction, RFparams,
      RFresults, i, j,
      meetingItemTypes, meetingItems,
      meetingSords, meetingGuid, votings,
  ...
  describe("create meeting items", function () {
    ...
    it("get configuration from meetingItemData.config", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_common_service_GetConfig", {
          objId: "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/meeting [unit tests]/Configuration/meetingItemData.config"
        }).then(function success(configResult) {
          meetingItemDataConfig = configResult.config;
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
    ...
    it("meetingDataConfig.meetingItems must be available", function () {
      expect(meetingItemDataConfig.meetingItems).toBeDefined();
    });
    ...
    it("start action create workflow for all meetingitems", function (done) {
      ...
      RFs = [];
      meetingItems = [];
      meetingItemDataConfig.meetingItems.forEach(function (meetingItem) {
        meetingGuid = 0;
        meetingSords.forEach(function (meetingSord) {
          if (meetingItem.objKeys.MEETING_BOARD_NAME == meetingSord.MEETING_BOARD_NAME) {
            if (meetingItem.objKeys.MEETING_NAME == meetingSord.MEETING_NAME) {
              meetingGuid = meetingSord.guid;
            }
          }
        });
