...
describe("[action] sol.meeting.ix.actions.CreateUpdateEvent", function () {
  var objTempId, meetingBoardTypes, configTypes, configAction, wfInfo, succNodes, succNodesIds,
      objIdMB, objIdM, objIdUE, originalTimeout;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.CreateUpdateEvent", null, null).then(function success(objTempId1) {
  ...
  describe("test create update event", function () {
  ...
  describe("test finish createupdateevent", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdM,
          $name: "CreateUpdateTeamsMeeting",
          $wf: {
            template: {
              name: "sol.meeting.ms365.event.CreateUpdate"
            },
            name: "{{translate 'sol.meeting.ms365.createupdate.prefix'}}"
          },
          $events: [
            {
              id: "DIALOG",
            ...
            },
            {
              id: "FEEDBACK",
              onWfStatus: "CREATED",
              message: "{{translate 'sol.meeting.ms365.client.feedback.eventCreatedUpdated'}}"
            }
          ]
        };
        wfInfo = {};
    ...
    it("fill update event", function (done) {
      expect(function () {
        objIdUE = wfInfo.objId;
        test.Utils.updateMapData(objIdUE, {
          MS365_EVENT_ORGANIZER_EMAIL_ADDRESS: "test-business-solutions@elo.local",
          MEETING_TIMESLOT_DAY1: null
        }).then(function success3(updateMapDataResult) {
          done();
        }, function error(err) {
          fail(err);
  ...
  describe("test cancel createupdateevent", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_common_action_Standard", configAction, []).then(function success(jsonResults) {
