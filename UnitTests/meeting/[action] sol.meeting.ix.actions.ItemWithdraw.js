...
describe("[action] sol.meeting.ix.actions.ItemWithdraw", function () {
  var objTempId, meetingBoardTypes, configTypes, configAction, wfInfo, succNodes, succNodesIds,
      objIdMB, objIdM, objIdMI, originalTimeout, meetingItemTypes, guidMIP;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.ItemWithdraw", null, null).then(function success(objTempId1) {
  ...
  describe("create meeting item pool 'default'", function () {
    ...
    it("fill meeting item pool sord", function (done) {
      expect(function () {
        test.Utils.getSord(wfInfo.objId).then(function success(sordMIP) {
          guidMIP = sordMIP.guid;
          test.Utils.updateKeywording(sordMIP, {
            MEETING_ITEM_POOL_NAME: "Testmeetingitempool"
          }, true).then(function success1(updateKeywordingResult) {
            done();
          }, function error(err) {
            fail(err);
  ...
  describe("test finish itemwithdraw", function () {
    it("check precondition withdraw meeting item", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_WithdrawMeetingItemPrecondition", { targetId: objIdMI }).then(function success(checkResult) {
    ...
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdMI,
          $name: "WithdrawMeetingItem",
          $wf: {
            template: {
              name: "sol.meeting.item.Withdraw"
            },
            name: "{{translate 'sol.meeting.item.withdraw.prefix'}}"
          },
          $events: [
            {
              id: "DIALOG",
            ...
            {
              id: "GOTO",
              onWfStatus: "WITHDRAWN"
            }
          ]
        };
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_meeting_service_WithdrawMeetingItem", configAction, []).then(function success(jsonResults) {
          test.Utils.handleAllEvents(jsonResults).then(function success1(wfInfo1) {
            wfInfo = wfInfo1;
            done();
          }, function error(err) {
    ...
    it("set meeting item pool", function (done) {
      expect(function () {
        test.Utils.updateMapData(objIdMI, {
          MEETING_ITEM_POOL_GUID: guidMIP
  ...
  describe("test cancel itemwithdraw", function () {
    it("check precondition withdraw meeting item", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_WithdrawMeetingItemPrecondition", { targetId: objIdMI }).then(function success(checkResult) {
          expect(checkResult.valid).toEqual(true);
