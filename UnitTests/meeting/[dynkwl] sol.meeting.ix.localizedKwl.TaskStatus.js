...
describe("[dynkwl] sol.meeting.ix.localizedKwl.TaskStatus", function () {
  var originalTimeout, objTaskStatusId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("TaskStatus").then(function success(objTaskStatusId1) {
        objTaskStatusId = objTaskStatusId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.meeting.task.ix.localizedKwl.TaskStatus", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteLib1", {
            className: "sol.meeting.task.ix.localizedKwl.TaskStatus",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.meeting.task.ix.localizedKwl.TaskStatus", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objTaskStatusId,
            dynKwl: "sol.meeting.task.ix.localizedKwl.TaskStatus"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
