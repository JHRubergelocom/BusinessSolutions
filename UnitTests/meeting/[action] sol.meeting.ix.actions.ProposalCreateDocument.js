...
describe("[action] sol.meeting.ix.actions.ProposalCreateDocument", function () {
  var objTempId, meetingBoardTypes, configTypes, configAction, wfInfo, succNodes, succNodesIds,
      objIdMB, objIdM, objIdMI, objIdPr, originalTimeout, meetingItemTypes, proposalTypes,
      interval;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.ProposalCreateDocument", null, null).then(function success(objTempId1) {
  ...
  describe("test propoasal create document", function () {
  ...
  describe("test finish proposal create document", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdPr,
          $name: "CreateProposalPDF",
          $wf: {
            template: {
              name: "sol.meeting_premium.proposal.CreateDocument"
            },
            name: "{{translate 'sol.meeting.client.ribbon.proposal.btnCreateDocument'}}"
          },
          $events: [
            {
              id: "DIALOG",
            ...
            },
            {
              id: "FEEDBACK",
              onWfStatus: "CREATE",
              message: "{{translate 'sol.meeting.client.proposal.feedback.createDocument'}}"
            },
            {
              id: "GOTO",
              onWfStatus: "CREATED"
          ...
          $metadata: {
            ...
            mapItems: [
              {
                key: "MEETING_PROPOSAL_DOCUMENTNAME",
                value: "{{{sord.name}}}"
              }
            ]
          }
    ...
    it("fill proposal create document", function (done) {
      expect(function () {
        test.Utils.updateMapData(wfInfo.objId, {
          MEETING_PROPOSAL_DOCUMENTNAME: "Besprechung Unittest",
          MEETING_PROPOSAL_VERSION_COMMENT: "Erste Version"
  ...
  describe("test cancel proposal create document", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_common_action_Standard", configAction, []).then(function success(jsonResults) {
