...
describe("[dynkwl] sol.meeting.ix.localizedKwl.CalendarUnitDeadline", function () {
  var originalTimeout, objCalendarUnitDeadlineId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CalendarUnitDeadline").then(function success(objCalendarUnitDeadlineId1) {
        objCalendarUnitDeadlineId = objCalendarUnitDeadlineId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.CalendarUnitDeadline", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objCalendarUnitDeadlineId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objCalendarUnitDeadlineId,
            dynKwl: "sol.meeting.ix.localizedKwl.CalendarUnitDeadline"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
