...
describe("[dynkwl] sol.meeting.ix.dynkwls.ResponsibleUserNames", function () {
  var originalTimeout, objResponsibleUserNamesId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ResponsibleUserNames").then(function success(objResponsibleUserNamesId1) {
        objResponsibleUserNamesId = objResponsibleUserNamesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.common.ix.dynkwl.UserNames", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objResponsibleUserNamesId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
