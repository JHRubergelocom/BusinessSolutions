...
describe("[action] sol.meeting.ix.actions.CreateMeetingMinutes", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.CreateMeetingMinutes", null, null).then(function success(objTempId1) {
  ...
  describe("test create meeting minutes", function () {
  ...
  describe("test finish create meeting minutes", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdM,
          $name: "CreateMeetingMinutes",
          $metadata: {
            solType: "MEETING",
            owner: {
              fromConnection: true
            ...
            mapItems: [
              {
                key: "MEETING_MINUTES_DOCUMENTNAME",
                value: "{{{sord.name}}}"
              },
              {
                key: "CURRENT_USER",
          ...
          $wf: {
            template: {
              name: "sol.meeting.CreateMeetingMinutes"
            },
            name: "{{translate 'sol.meeting.createMeetingMinutes.prefix'}}"
          },
          $events: [
            {
              id: "DIALOG",
            ...
            },
            {
              id: "FEEDBACK",
              onWfStatus: "CREATE",
              message: "{{translate 'sol.meeting.client.meetingMinutes.feedback.create'}}"
            },
            {
              id: "GOTO",
              onWfStatus: "CREATED"
    ...
    it("Configure meeting minutes", function (done) {
      expect(function () {
        test.Utils.updateMapData(objIdM, {
          MEETING_MINUTES_DOCUMENTNAME: "Test Besprechungsprotokoll",
          MEETING_MINUTES_VERSION_COMMENT: "Neue Version"
        }).then(function success(updateMapDataResult) {
          test.Utils.updateWfMapData(wfInfo.flowId, objIdM, {
            SETTING_MINUTES_CREATE_PARTIAL: "",
            SETTING_MINUTES_CREATE_TOTAL: "1"
          }).then(function success1(updateWfMapDataResult) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
  ...
  describe("test cancel create meeting minutes", function () {
    it("start action create workflow", function (done) {
      expect(function () {
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_common_action_Standard", configAction, []).then(function success(jsonResults) {
