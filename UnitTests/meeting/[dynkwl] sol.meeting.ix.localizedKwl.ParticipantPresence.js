...
describe("[dynkwl] sol.meeting.ix.localizedKwl.ParticipantPresence", function () {
  var originalTimeout, objParticipantPresenceId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ParticipantPresence").then(function success(objParticipantPresenceId1) {
        objParticipantPresenceId = objParticipantPresenceId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.ParticipantPresence", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objParticipantPresenceId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objParticipantPresenceId,
            dynKwl: "sol.meeting.ix.localizedKwl.ParticipantPresence"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
