...
describe("[action] sol.meeting.ix.actions.ItemRegister", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.ItemRegister", null, null).then(function success(objTempId1) {
  ...
  describe("test item register", function () {
  ...
  describe("test finish itemregister", function () {
    it("check precondition register meeting item", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_RegisterMeetingItemPrecondition", { targetId: objIdMI }).then(function success(checkResult) {
          expect(checkResult.msg).toBeDefined();
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
    ...
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdMI,
          $name: "RegisterMeetingItem",
          $wf: {
            template: {
              name: "sol.meeting.item.Register"
            },
            name: "{{translate 'sol.meeting.item.register.prefix'}}-{{formatDate 'YYYYMMDDHHmmss'}}"
          },
          $events: [
            {
              id: "DIALOG",
  ...
  describe("test cancel itemregister", function () {
    it("check precondition register meeting item", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_RegisterMeetingItemPrecondition", { targetId: objIdMI }).then(function success(checkResult) {
          expect(checkResult.msg).toBeDefined();
