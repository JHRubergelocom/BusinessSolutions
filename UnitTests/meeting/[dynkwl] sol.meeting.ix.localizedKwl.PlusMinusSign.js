...
describe("[dynkwl] sol.meeting.ix.localizedKwl.PlusMinusSign", function () {
  var originalTimeout, objPlusMinusSignId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PlusMinusSign").then(function success(objPlusMinusSignId1) {
        objPlusMinusSignId = objPlusMinusSignId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.PlusMinusSign", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objPlusMinusSignId,
            dynKwl: "sol.meeting.ix.localizedKwl.PlusMinusSign"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
