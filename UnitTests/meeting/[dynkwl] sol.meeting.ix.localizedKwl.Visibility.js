...
describe("[dynkwl] sol.meeting.ix.localizedKwl.Visibility", function () {
  var originalTimeout, objVisibilityId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Visibility").then(function success(objVisibilityId1) {
        objVisibilityId = objVisibilityId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.meeting.note.ix.localizedKwl.Visibility", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteLib1", {
            className: "sol.meeting.note.ix.localizedKwl.Visibility",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.meeting.note.ix.localizedKwl.Visibility", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objVisibilityId,
            dynKwl: "sol.meeting.note.ix.localizedKwl.Visibility"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
