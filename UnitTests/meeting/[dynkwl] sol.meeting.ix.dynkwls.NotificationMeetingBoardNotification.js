...
describe("[dynkwl] sol.meeting.ix.dynkwls.NotificationMeetingBoardNotification", function () {
  var originalTimeout, objNotificationMeetingBoardNotificationId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("NotificationMeetingBoardNotification").then(function success(objNotificationMeetingBoardNotificationId1) {
        objNotificationMeetingBoardNotificationId = objNotificationMeetingBoardNotificationId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.notification.meetingBoard.notification", function () {
      it("get 'MeetingBoardNotificationTemplates' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["MeetingBoardNotificationTemplates"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objNotificationMeetingBoardNotificationId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
