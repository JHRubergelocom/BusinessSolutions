...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingContact", function () {
  var originalTimeout, objParticipantContactsId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MeetingContact").then(function success(objParticipantContactsId1) {
        objParticipantContactsId = objParticipantContactsId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.MeetingContact", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objParticipantContactsId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
