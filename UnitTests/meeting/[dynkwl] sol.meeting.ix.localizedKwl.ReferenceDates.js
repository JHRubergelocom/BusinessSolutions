...
describe("[dynkwl] sol.meeting.ix.localizedKwl.ReferenceDates", function () {
  var originalTimeout, objReferenceDatesId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ReferenceDates").then(function success(objReferenceDatesId1) {
        objReferenceDatesId = objReferenceDatesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.ReferenceDates", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objReferenceDatesId,
            dynKwl: "sol.meeting.ix.localizedKwl.ReferenceDates"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
