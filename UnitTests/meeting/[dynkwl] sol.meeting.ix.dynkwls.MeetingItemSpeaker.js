...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingItemSpeaker", function () {
  var originalTimeout, objMeetingItemSpeakerId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MeetingItemSpeaker").then(function success(objMeetingItemSpeakerId1) {
        objMeetingItemSpeakerId = objMeetingItemSpeakerId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.meeting.ix.dynkwl.MeetingItemSpeaker", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objMeetingItemSpeakerId,
            dynKwl: "sol.meeting.ix.dynkwl.MeetingItemSpeaker"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
