...
describe("[dynkwl] sol.meeting.ix.dynkwls.MeetingIterator", function () {
  var originalTimeout, objMeetingIteratorId, filterList,
      sord, config;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("MeetingIterator").then(function success(objMeetingIteratorId1) {
        objMeetingIteratorId = objMeetingIteratorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.meeting.ix.dynkwl.MeetingIterator", function () {
      it("getFindInfo", function (done) {
        expect(function () {
          filterList = [];
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteLib1", {
            className: "sol.meeting.ix.dynkwl.MeetingIterator",
            classConfig: {},
            method: "getFindInfo",
            params: [filterList]
    ...
    describe("sol.meeting.ix.dynkwl.MeetingIterator", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_meeting_service_ExecuteDynKwl", {
            objId: objMeetingIteratorId,
            dynKwl: "sol.meeting.ix.dynkwl.MeetingIterator"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
