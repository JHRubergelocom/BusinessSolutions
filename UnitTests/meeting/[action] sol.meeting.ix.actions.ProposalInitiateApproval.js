...
describe("[action] sol.meeting.ix.actions.ProposalInitiateApproval", function () {
  var objTempId, meetingBoardTypes, configTypes, configAction, wfInfo, succNodes, succNodesIds,
      ...
      userNode, nodes, userNodeId, interval;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.ProposalInitiateApproval", null, null).then(function success(objTempId1) {
  ...
  describe("test finish proposalinitiateapproval without DynAdHoc Approval", function () {
    it("check precondition proposal approval", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_ProposalPrecondition", {
          targetId: objIdPr
        }).then(function success(checkResult) {
          expect(checkResult.valid).toBeDefined();
          expect(checkResult.valid).toEqual(true);
          expect(checkResult.targetId).toBeDefined();
          expect(checkResult.targetId).toEqual(objIdPr);
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
    ...
    it("start action create workflow", function (done) {
      expect(function () {
        configAction = {
          ...
          $wf: {
            template: {
              name: "sol.meeting_premium.proposal.Approval"
            },
  ...
  describe("test finish proposalinitiateapproval -> 'OK' -> 'Freigeben' -> 'Bestätigen", function () {
    it("check precondition proposal approval", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_ProposalPrecondition", {
          targetId: objIdPr
          ...
          expect(checkResult.valid).toBeDefined();
          expect(checkResult.valid).toEqual(false);
          expect(checkResult.targetId).toBeDefined();
          expect(checkResult.targetId).toEqual(objIdPr);
          done();
        }, function error(err) {
  ...
  describe("test finish proposalinitiateapproval -> 'OK' -> 'Ablehnen' -> 'Bestätigen", function () {
    it("check precondition proposal approval", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_ProposalPrecondition", {
          targetId: objIdPr
  ...
  describe("test cancel proposalinitiateapproval", function () {
    it("check precondition proposal approval", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_meeting_service_ProposalPrecondition", {
          targetId: objIdPr
