...
describe("[action] sol.visitor.ix.actions.CheckOutVisitor", function () {
  var objTempId, objIdVs1, objIdVs2, objIdGr1, objIdGr2, objIdVsGr,
      visitorTypes, config, objIdConfigVisitor, configVisitor,
      checkoutVisitorWithoutDialogOld,
      wfInfo, keywording, mapdata, nowDateTime,
  ...
  describe("test single visitor", function () {
      ...
      it("finish workflow", function (done) {
        expect(function () {
          test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
            succNodes = test.Utils.getSuccessorNodes(workflow, wfInfo.nodeId, null, "Create and check in visitor");
    ...
    describe("test finish checkoutvisitor", function () {
      ...
      it("finish input forwarding workflow", function (done) {
        expect(function () {
          test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
            succNodes = test.Utils.getSuccessorNodes(workflow, wfInfo.nodeId, null, "Check out visitor");
            succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
            test.Utils.forwardWorkflowTask(wfInfo.flowId, wfInfo.nodeId, succNodesIds, "Unittest finish input").then(function success1(forwardWorkflowTaskResult) {
              done();
            }, function error(err) {
  ...
  describe("test group", function () {
    describe("register group1", function () {
      ...
      it("finish workflow", function (done) {
        expect(function () {
          test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
            succNodes = test.Utils.getSuccessorNodes(workflow, wfInfo.nodeId, null, "Create and check in group");
            succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
            test.Utils.forwardWorkflowTask(wfInfo.flowId, wfInfo.nodeId, succNodesIds, "Unittest finish input").then(function success1(forwardWorkflowTaskResult) {
              done();
            }, function error(err) {
    ...
    describe("test cancel checkoutgroup", function () {
      it("get current date, time", function () {
        expect(function () {
          nowDateTime = test.Utils.getNowDateTime();
        }).not.toThrow();
      });
      it("get visitor in group", function (done) {
        expect(function () {
          test.Utils.findChildren(objIdGr1).then(function success(childrenFolder) {
            expect(childrenFolder.length).toEqual(1);
            objIdVsGr = childrenFolder[0].id;
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("set departure date group sord", function (done) {
        expect(function () {
          test.Utils.getSord(objIdGr1).then(function success(sordGr1) {
            keywording = {
              VISITOR_DEPARTUREDATE: nowDateTime.date
            };
            test.Utils.updateKeywording(sordGr1, keywording, true).then(function success1(updateKeywordingResult) {
      ...
      it("set checkout and depature time single visitor1", function (done) {
        expect(function () {
          mapdata = {
            VISITOR_DEPARTURETIME1: nowDateTime.time,
            VISITOR_CHECKOUTVISITOR1: 1
          };
          test.Utils.updateMapData(objIdGr1, mapdata).then(function success(updateMapDataResult) {
            done();
          }, function error(err) {
    ...
    describe("test finish checkoutgroup", function () {
      it("get current date, time", function () {
        expect(function () {
          nowDateTime = test.Utils.getNowDateTime();
        }).not.toThrow();
      ...
      it("get visitor in group", function (done) {
        expect(function () {
          test.Utils.findChildren(objIdGr2).then(function success(childrenFolder) {
            expect(childrenFolder.length).toEqual(1);
            objIdVsGr = childrenFolder[0].id;
            done();
          }, function error(err) {
      ...
      it("set departure date group sord", function (done) {
        expect(function () {
          test.Utils.getSord(objIdGr2).then(function success(sordGr2) {
            keywording = {
              VISITOR_DEPARTUREDATE: nowDateTime.date
            };
            test.Utils.updateKeywording(sordGr2, keywording, true).then(function success1(updateKeywordingResult) {
      ...
      it("set checkout and depature time in single visitor1 and visitor2", function (done) {
        expect(function () {
          mapdata = {
            VISITOR_CHECKOUTVISITOR1: 1,
            VISITOR_CHECKOUTVISITOR2: 1,
            VISITOR_DEPARTURETIME1: nowDateTime.time,
            VISITOR_DEPARTURETIME2: nowDateTime.time
          };
          test.Utils.updateMapData(objIdGr2, mapdata).then(function success(updateMapDataResult) {
            done();
          }, function error(err) {
