...
describe("[dynkwl] sol.visitor.ix.localizedKwl.Category", function () {
  var originalTimeout, objCategoryId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Category").then(function success(objCategoryId1) {
        objCategoryId = objCategoryId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.visitor.ix.localizedKwl.Category", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteLib1", {
            className: "sol.visitor.ix.localizedKwl.Category",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.Category", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objCategoryId,
            dynKwl: "sol.visitor.ix.localizedKwl.Category"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
