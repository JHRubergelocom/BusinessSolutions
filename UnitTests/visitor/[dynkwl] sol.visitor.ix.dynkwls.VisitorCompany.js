...
describe("[dynkwl] sol.visitor.ix.dynkwls.VisitorCompany", function () {
  var originalTimeout, objVisitorCompanyId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("VisitorCompany").then(function success(objVisitorCompanyId1) {
        objVisitorCompanyId = objVisitorCompanyId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.VisitorCompany", function () {
      it("get 'Visitor companies' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor companies"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
