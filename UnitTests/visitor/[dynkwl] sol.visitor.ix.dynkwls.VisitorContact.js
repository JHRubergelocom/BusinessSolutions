...
describe("[dynkwl] sol.visitor.ix.dynkwls.VisitorContact", function () {
  var originalTimeout, objVisitorContactId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("VisitorContact").then(function success(objVisitorContactId1) {
        objVisitorContactId = objVisitorContactId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.VisitorContact", function () {
      it("get 'Visitor contacts' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor contacts"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
