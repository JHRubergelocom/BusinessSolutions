...
describe("[dynkwl] sol.visitor.ix.dynkwls.generators.NameVisitor", function () {
  var originalTimeout, objgeneratorsNameVisitorId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsNameVisitor").then(function success(objgeneratorsNameVisitorId1) {
        objgeneratorsNameVisitorId = objgeneratorsNameVisitorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.generators.NameVisitor", function () {
      it("get 'Visitor name generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor name generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objgeneratorsNameVisitorId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
