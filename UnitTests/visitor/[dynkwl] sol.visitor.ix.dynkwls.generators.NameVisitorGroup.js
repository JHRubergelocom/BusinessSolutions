...
describe("[dynkwl] sol.visitor.ix.dynkwls.generators.NameVisitorGroup", function () {
  var originalTimeout, objgeneratorsNameVisitorGroupId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsNameVisitorGroup").then(function success(objgeneratorsNameVisitorGroupId1) {
        objgeneratorsNameVisitorGroupId = objgeneratorsNameVisitorGroupId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.generators.NameVisitorGroup", function () {
      it("get 'Visitor group name generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor group name generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objgeneratorsNameVisitorGroupId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
