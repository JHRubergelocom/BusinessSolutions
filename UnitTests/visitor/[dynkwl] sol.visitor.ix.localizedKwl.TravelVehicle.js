...
describe("[dynkwl] sol.visitor.ix.localizedKwl.TravelVehicle", function () {
  var originalTimeout, objTravelVehicleId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("TravelVehicle").then(function success(objTravelVehicleId1) {
        objTravelVehicleId = objTravelVehicleId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.visitor.ix.localizedKwl.TravelVehicle", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteLib1", {
            className: "sol.visitor.ix.localizedKwl.TravelVehicle",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.TravelVehicle", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objTravelVehicleId,
            dynKwl: "sol.visitor.ix.localizedKwl.TravelVehicle"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
