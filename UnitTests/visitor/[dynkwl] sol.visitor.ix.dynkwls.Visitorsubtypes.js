...
describe("[dynkwl] sol.visitor.ix.dynkwls.Visitorsubtypes", function () {
  var originalTimeout, objVisitorsubtypesId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Visitorsubtypes").then(function success(objVisitorsubtypesId1) {
        objVisitorsubtypesId = objVisitorsubtypesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.Visitorsubtypes", function () {
      it("get 'Visitor subtypes' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor subtypes"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
