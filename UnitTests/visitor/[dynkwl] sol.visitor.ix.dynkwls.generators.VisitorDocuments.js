...
describe("[dynkwl] sol.visitor.ix.dynkwls.generators.VisitorDocuments", function () {
  var originalTimeout, objgeneratorsVisitorDocumentsId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsVisitorDocuments").then(function success(objgeneratorsVisitorDocumentsId1) {
        objgeneratorsVisitorDocumentsId = objgeneratorsVisitorDocumentsId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.generators.VisitorDocuments", function () {
      it("get 'Visitor documents' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor documents"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objgeneratorsVisitorDocumentsId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
