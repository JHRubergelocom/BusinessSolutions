...
describe("[dynkwl] sol.visitor.ix.dynkwls.generators.ReferenceLTB", function () {
  var originalTimeout, objgeneratorsReferenceLTBId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsReferenceLTB").then(function success(objgeneratorsReferenceLTBId1) {
        objgeneratorsReferenceLTBId = objgeneratorsReferenceLTBId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.generators.ReferenceLTB", function () {
      it("get 'Longterm badge reference generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Longterm badge reference generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objgeneratorsReferenceLTBId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
