...
describe("[dynkwl] sol.visitor.ix.dynkwls.VisitorListTemplate", function () {
  var originalTimeout, objVisitorListTemplateId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("VisitorListTemplate").then(function success(objVisitorListTemplateId1) {
        objVisitorListTemplateId = objVisitorListTemplateId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.VisitorListTemplate", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_visitor_service_ExecuteDynKwl", {
            objId: objVisitorListTemplateId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
