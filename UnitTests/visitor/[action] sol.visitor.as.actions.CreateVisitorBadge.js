...
describe("[action] sol.visitor.as.actions.CreateVisitorBadge", function () {
  ...
  describe("capture picture visitor", function () {
    ...
    it("finish input forwarding workflow", function (done) {
      expect(function () {
        test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
          succNodes = test.Utils.getSuccessorNodes(workflow, wfInfo.nodeId, null, "Capture visitor photo");
          succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
          test.Utils.forwardWorkflowTask(wfInfo.flowId, wfInfo.nodeId, succNodesIds, "Unittest finish input").then(function success1(forwardWorkflowTaskResult) {
            done();
          }, function error(err) {
  ...
  describe("test create visitorbadge visitor", function () {
    ...
    it("visitorBadgeTypes must be available", function (done) {
      test.Utils.execute("RF_sol_visitor_service_GetVisitorBadgeTypes", {}).then(function success(visitorBadgeTypes1) {
        visitorBadgeTypes = visitorBadgeTypes1;
