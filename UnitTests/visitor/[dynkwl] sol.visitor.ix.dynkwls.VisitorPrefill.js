...
describe("[dynkwl] sol.visitor.ix.dynkwls.VisitorPrefill", function () {
  var originalTimeout, objVisitorPrefillId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("VisitorPrefill").then(function success(objVisitorPrefillId1) {
        objVisitorPrefillId = objVisitorPrefillId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.visitor.ix.dynkwl.VisitorPrefill", function () {
      it("get 'Visitor prefill' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Visitor prefill"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
