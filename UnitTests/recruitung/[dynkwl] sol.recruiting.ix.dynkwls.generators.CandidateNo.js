...
describe("[dynkwl] sol.recruiting.ix.dynkwls.generators.CandidateNo", function () {
  var originalTimeout, objGeneratorsCandidateNoId,
      config;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("GeneratorsCandidateNo").then(function success(objGeneratorsCandidateNoId1) {
        objGeneratorsCandidateNoId = objGeneratorsCandidateNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.dynkwl.generators.CandidateNo", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.dynkwl.generators.CandidateNo",
            classConfig: {},
            method: "initialize",
            params: [config]
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.generators.CandidateNo", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objGeneratorsCandidateNoId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objGeneratorsCandidateNoId,
            dynKwl: "sol.recruiting.ix.dynkwl.generators.CandidateNo"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
