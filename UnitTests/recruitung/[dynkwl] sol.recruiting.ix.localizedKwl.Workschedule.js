...
describe("[dynkwl] sol.recruiting.ix.localizedKwl.Workschedule", function () {
  var originalTimeout, objWorkscheduleId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Workschedule").then(function success(objWorkscheduleId1) {
        objWorkscheduleId = objWorkscheduleId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.localizedKwl.Workschedule", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.localizedKwl.Workschedule",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.Workschedule", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objWorkscheduleId,
            dynKwl: "sol.recruiting.ix.localizedKwl.Workschedule"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
