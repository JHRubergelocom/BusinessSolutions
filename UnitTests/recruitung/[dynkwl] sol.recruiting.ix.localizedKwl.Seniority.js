...
describe("[dynkwl] sol.recruiting.ix.localizedKwl.Seniority", function () {
  var originalTimeout, objSeniorityId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Seniority").then(function success(objSeniorityId1) {
        objSeniorityId = objSeniorityId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.Seniority", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objSeniorityId,
            dynKwl: "sol.recruiting.ix.localizedKwl.Seniority"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
