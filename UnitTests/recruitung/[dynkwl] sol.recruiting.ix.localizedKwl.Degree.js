...
describe("[dynkwl] sol.recruiting.ix.localizedKwl.Degree", function () {
  var originalTimeout, objDegreeId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Degree").then(function success(objDegreeId1) {
        objDegreeId = objDegreeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.localizedKwl.Degree", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.localizedKwl.Degree",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.Degree", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objDegreeId,
            dynKwl: "sol.recruiting.ix.localizedKwl.Degree"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
