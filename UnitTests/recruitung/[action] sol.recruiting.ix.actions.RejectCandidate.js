...
describe("[action] sol.recruiting.ix.actions.RejectCandidate", function () {
  var objTempId, candidateTypes,
      configTypes, configAction, wfInfo, succNodes, succNodesIds, objIdC1, objIdC2,
      privateEmail, objIdR, requisitionTypes, rejectionTypes,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.RejectCandidate", null, null).then(function success(objTempId1) {
  ...
  describe("test candidate reject", function () {
    ...
    describe("test finish rejectcandidate", function () {
      it("rejectionTypes must be available", function (done) {
        configTypes = {
          $types: {
        ...
        test.Utils.execute("RF_sol_common_service_StandardTypes", configTypes).then(function success(rejectionTypes1) {
          rejectionTypes = rejectionTypes1;
          expect(rejectionTypes).toBeDefined();
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
      ...
      it("rejectionTypes.length must greater than zero", function () {
        expect(rejectionTypes.length).toBeGreaterThan(0);
      });
      ...
      it("start action candidate reject workflow", function (done) {
        expect(function () {
          configAction = {
            ...
            $wf: {
              template: {
                name: "sol.recruiting.Candidate.Reject"
              },
              name: "sol.recruiting.candidate.workflow.rejectcandidate.message"
            },
            $events: [
              {
                id: "DIALOG"
              },
              {
                id: "FEEDBACK",
                onWfStatus: "SENT",
                message: "sol.recruiting.candidate.workflow.rejectcandidate.feedback"
              }
            ],
            $new: {
              target: {
              ...
              name: "Reject scheduling",
              template: {
                base: "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/recruiting/Configuration/Communication Templates/Rejection",
                name: rejectionTypes[0].name
              }
            },
            $permissions: {
              mode: "SET",
    ...
    describe("test cancel rejectcandidate", function () {
      it("rejectionTypes must be available", function (done) {
        configTypes = {
          $types: {
            path: "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/recruiting/Configuration/Communication Templates/Rejection"
          ...
          test.Utils.deleteSord(objIdC1).then(function success2(deleteResult1) {
            test.Utils.deleteSord(objIdC2).then(function success3(deleteResult2) {
              test.Utils.deleteSord(objIdR).then(function success4(deleteResult3) {
                test.Utils.getActiveWorkflows().then(function success5(wfs) {
                  test.Utils.removeActiveWorkflows(wfs).then(function success6(removeFinishedWorkflowsResult) {
                    done();
                  }, function error(err) {
                    fail(err);
                    console.error(err);
