...
describe("[action] sol.recruiting.ix.actions.HireCandidate", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.HireCandidate", null, null).then(function success(objTempId1) {
  ...
  describe("test hire candidate", function () {
    ...
    describe("test finish hirecandidate", function () {
      it("start action hire candidate workflow", function (done) {
        expect(function () {
          configAction = {
            $name: "HireCandidate",
            objId: objIdC1,
            ...
            $wf: {
              template: {
                name: "sol.recruiting.Candidate.Hire"
              },
              name: "sol.recruiting.candidate.workflow.hire.message"
            },
            $events: [
              {
                id: "DIALOG"
      ...
      it("Prepare Personalfile", function (done) {
        expect(function () {
          test.Utils.updateWfMapData(wfInfo.flowId, objIdC1, { RECRUITING_FILE_TEMPLATE: "Default" }).then(function success(updateMapDataResult) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("remove workflows", function (done) {
        expect(function () {
          test.Utils.getFinishedWorkflows().then(function success(wfs) {
            test.Utils.removeFinishedWorkflows(wfs).then(function success1(removeFinishedWorkflowsResult) {
              if (wfs.length > 1) {
                test.Utils.deleteSord(wfs[1].objId).then(function success2(deleteResult) {
                  done();
                }, function error(err) {
                  fail(err);
                  console.error(err);
    ...
    describe("test cancel hirecandidate", function () {
      it("start action hire candidate workflow", function (done) {
        expect(function () {
          configAction = {
            $name: "HireCandidate",
