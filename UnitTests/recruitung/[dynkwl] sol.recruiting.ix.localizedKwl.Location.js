...
describe("[dynkwl] sol.recruiting.ix.localizedKwl.Location", function () {
  var originalTimeout, objLocationId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Location").then(function success(objLocationId1) {
        objLocationId = objLocationId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.localizedKwl.Location", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.localizedKwl.Location",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.Location", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objLocationId,
            dynKwl: "sol.recruiting.ix.localizedKwl.Location"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
