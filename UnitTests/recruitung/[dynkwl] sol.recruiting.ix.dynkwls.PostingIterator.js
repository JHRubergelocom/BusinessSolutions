...
describe("[dynkwl] sol.recruiting.ix.dynkwls.PostingIterator", function () {
  var originalTimeout, objPostingIteratorId,
      filterList, sord, config;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PostingIterator").then(function success(objPostingIteratorId1) {
        objPostingIteratorId = objPostingIteratorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.dynkwl.PostingIterator", function () {
      it("getFindInfo", function (done) {
        filterList = [];
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.dynkwl.PostingIterator",
            classConfig: {},
            method: "getFindInfo",
            params: [filterList]
    ...
    describe("sol.recruiting.ix.dynkwl.PostingIterator", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objPostingIteratorId,
            dynKwl: "sol.recruiting.ix.dynkwl.PostingIterator"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
