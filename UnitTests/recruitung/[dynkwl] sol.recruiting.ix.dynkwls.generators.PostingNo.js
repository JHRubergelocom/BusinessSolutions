...
describe("[dynkwl] sol.recruiting.ix.dynkwls.generators.PostingNo", function () {
  var originalTimeout, objGeneratorsPostingNoId,
      config;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("GeneratorsPostingNo").then(function success(objGeneratorsPostingNoId1) {
        objGeneratorsPostingNoId = objGeneratorsPostingNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.recruiting.ix.dynkwl.generators.PostingNo", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteLib1", {
            className: "sol.recruiting.ix.dynkwl.generators.PostingNo",
            classConfig: {},
            method: "initialize",
            params: [config]
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.recruiting.ix.dynkwl.generators.PostingNo", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objGeneratorsPostingNoId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_recruiting_service_ExecuteDynKwl", {
            objId: objGeneratorsPostingNoId,
            dynKwl: "sol.recruiting.ix.dynkwl.generators.PostingNo"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
