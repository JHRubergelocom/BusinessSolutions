...
describe("[function] sol.common.ix.functions.CheckFromService", function () {
  var originalTimeout, config, objCheckFromServiceId,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("CheckFromService").then(function success(objCheckFromServiceId1) {
        objCheckFromServiceId = objCheckFromServiceId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.common.ix.functions.CheckFromService", function () {
      it("start Workflow Unittest", function (done) {
        expect(function () {
          test.Utils.startWorkflow("Unittest", "Workflow Unittest", objCheckFromServiceId).then(function success(flowId1) {
            flowId = flowId1;
            done();
          }, function error(err) {
            fail(err);
      ...
      it("initialize", function (done) {
        expect(function () {
          ...
          test.Utils.execute("RF_sol_unittest_service_ExecuteLib8", {
            className: "sol.common.ix.functions.CheckFromService",
