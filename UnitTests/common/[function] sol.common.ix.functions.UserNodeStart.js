...
describe("[function] sol.common.ix.functions.UserNodeStart", function () {
  var originalTimeout, config, objUserNodeStartId,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("UserNodeStart").then(function success(objUserNodeStartId1) {
        objUserNodeStartId = objUserNodeStartId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.common.ix.functions.UserNodeStart", function () {
      it("start Workflow Unittest", function (done) {
        expect(function () {
          test.Utils.startWorkflow("Unittest", "Workflow Unittest", objUserNodeStartId).then(function success(flowId1) {
            flowId = flowId1;
            done();
          }, function error(err) {
            fail(err);
      ...
      it("initialize", function (done) {
        expect(function () {
          ...
          test.Utils.execute("RF_sol_unittest_service_ExecuteLib8", {
            className: "sol.common.ix.functions.UserNodeStart",
            classConfig: { objId: objUserNodeStartId, wfDiagram: workflow },
            method: "initialize",
            params: [config]
