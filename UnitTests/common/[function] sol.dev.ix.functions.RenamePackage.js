...
describe("[function] sol.dev.ix.functions.RenamePackage", function () {
  var packageTypes,
      configAction, wfInfo,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("RenamePackage", null, null).then(function success(objTempId) {
        interval = 4000;
        done();
      }, function error(err) {
        fail(err);
  ...
  describe("test cases rename package", function () {
    it("set package mapfield data", function (done) {
      expect(function () {
        test.Utils.updateMapData(objPackageId, { NAMESPACE: "sol", PACKAGE: "unittest", FULLNAMESPACE: "unittest", DESCRIPTION: "Description Unittest" }).then(function success(updateMapDataResult) {
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
    ...
    it("should throw if executed without parameter 'objId'", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_dev_function_RenamePackage", {
    ...
    it("rename package", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_dev_function_RenamePackage", {
          objId: objPackageId
        }).then(function success(jsonResult) {
