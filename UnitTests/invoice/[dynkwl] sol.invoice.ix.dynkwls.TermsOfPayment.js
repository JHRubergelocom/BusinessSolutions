...
describe("[dynkwl] sol.invoice.ix.dynkwls.TermsOfPayment", function () {
  var originalTimeout, objTermsOfPaymentId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("TermsOfPayment").then(function success(objTermsOfPaymentId1) {
        objTermsOfPaymentId = objTermsOfPaymentId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.TermsOfPayment", function () {
      it("get 'Terms of payment' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Terms of payment"];
            kwlConfig.searchParams = ["1000", "61000"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
