...
describe("[action] sol.invoice.SearchVendor", function () {
  var objIncomingInvoiceId, entryFolder, targetId,
      ...
      sourceId, i, invoiceMask, params, field, searchTerm, objKeysObj,
      originalTimeout;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.SearchVendor", null, null).then(function success(objTempId) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
    ...
    it("set invoice date, invoice number, purchase user, vendor", function (done) {
      expect(function () {
        test.Utils.getSord(objIncomingInvoiceId).then(function success(IncomingInvoiceSord) {
          test.Utils.updateKeywording(IncomingInvoiceSord, { INVOICE_DATE: "20080101", INVOICE_NUMBER: "99999", PO_PURCHASE_USER: "Administrator", VENDOR_NO: "77777" }, true).then(function success1(updateKeywordingResult) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
  ...
  describe("test search vendor", function () {
    it("Start Search Vendor", function (done) {
      expect(function () {
        test.Utils.getSord(objIncomingInvoiceId).then(function success(IncomingInvoiceSord) {
          field = invoiceConfig.config.fields.VENDOR_NO.value;
          searchTerm = "*";
          expect(field).toBeDefined();
          expect(searchTerm).toBeDefined();
          objKeysObj = {};
          objKeysObj[field] = searchTerm;
          params = { objKeysObj: objKeysObj, maskId: maskId };
