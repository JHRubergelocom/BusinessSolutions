...
describe("[dynkwl] sol.invoice.ix.localizedKwl.DocType", function () {
  var originalTimeout, objDocTypeId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("DocType").then(function success(objDocTypeId1) {
        objDocTypeId = objDocTypeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.DocType", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_invoice_service_ExecuteDynKwl", {
            objId: objDocTypeId,
            dynKwl: "sol.invoice.ix.localizedKwl.DocType"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
