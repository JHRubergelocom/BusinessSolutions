...
describe("[dynkwl] sol.invoice.ix.dynkwls.PurchaseOrderNoLines", function () {
  var originalTimeout, objPurchaseOrderNoLinesId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PurchaseOrderNoLines").then(function success(objPurchaseOrderNoLinesId1) {
        objPurchaseOrderNoLinesId = objPurchaseOrderNoLinesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.PurchaseOrderNoLines", function () {
      it("get 'Purchase order no lines' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Purchase order no lines"];
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_invoice_service_ExecuteDynKwl", {
            objId: objPurchaseOrderNoLinesId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
