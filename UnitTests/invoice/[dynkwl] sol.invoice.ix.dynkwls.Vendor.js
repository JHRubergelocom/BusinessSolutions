...
describe("[dynkwl] sol.invoice.ix.dynkwls.Vendor", function () {
  var originalTimeout, objVendorId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Vendor").then(function success(objVendorId1) {
        objVendorId = objVendorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.Vendor", function () {
      it("get 'Vendors' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Vendors"];
            kwlConfig.searchParams = ["1000", "C", "C", "C", "C"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
