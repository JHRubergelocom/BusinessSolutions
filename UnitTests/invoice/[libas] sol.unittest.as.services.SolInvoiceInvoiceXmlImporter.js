...
describe("[libas] sol.unittest.as.services.SolInvoiceInvoiceXmlImporter", function () {
  var originalTimeout, content, invoiceConfig, objId, obTempIncomingInvoiceId,
      ...
      succNodes, succNodesIds, interval, i,
  ...
  describe("Test Lib Functions", function () {
    describe("sol.invoice.as.InvoiceXmlImporter", function () {
      ...
      it("getInvoiceWorkflow", function (done) {
        expect(function () {
          test.Utils.getActiveWorkflows().then(function success1(wfs) {
            invoiceFlowId = wfs[0].id;
            done();
          }, function error(err) {
            console.error(err);
            done();
      ...
      it("set invoice date, invoice number, purchase user, vendor", function (done) {
        expect(function () {
          test.Utils.getSord(obTempIncomingInvoiceId).then(function success(IncomingInvoiceSord) {
      ...
      it("remove incoming invoice", function (done) {
        expect(function () {
          test.Utils.deleteSord(obTempIncomingInvoiceId).then(function success2(deleteResult) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
