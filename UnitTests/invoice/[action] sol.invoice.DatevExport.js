...
describe("[action] sol.invoice.DatevExport", function () {
  var objIncomingInvoiceId, entryFolder, targetId,
      ...
      sourceId, i, invoiceMask, language,
      selectedSordInvoicesGuid, params, originalTimeout;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.DatevExport", null, null).then(function success(objTempId) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("test datev export", function () {
    it("Start Datev-Export", function (done) {
      expect(function () {
        test.Utils.getSord(objIncomingInvoiceId).then(function success(IncomingInvoiceSord) {
          selectedSordInvoicesGuid = [];
          selectedSordInvoicesGuid.push(IncomingInvoiceSord.guid + "");
          params = { objIds: selectedSordInvoicesGuid, language: language };

          test.Utils.executeASActionHandler("invoice", "sol.invoice_datev.Export", params, []).then(function success1(jsonResults) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
