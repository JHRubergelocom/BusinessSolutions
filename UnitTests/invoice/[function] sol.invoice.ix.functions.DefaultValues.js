...
describe("[function] sol.invoice.ix.functions.DefaultValues", function () {
  var invoiceMask, invoiceConfig,
      ...
      i, objIncomingInvoiceId, params, originalTimeout;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Functions.DefaultValues", null, null).then(function success(objTempId) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("test set default values", function () {
    it("should not throw if executed without 'objId'", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_invoice_setDefaultValues", {
    ...
    it("should not throw", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_invoice_setDefaultValues", {
          ...
          test.Utils.getSord(objIncomingInvoiceId).then(function success1(sordInvoice) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
