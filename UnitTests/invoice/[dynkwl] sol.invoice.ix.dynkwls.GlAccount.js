...
describe("[dynkwl] sol.invoice.ix.dynkwls.GlAccount", function () {
  var originalTimeout, objGlAccountId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("GlAccount").then(function success(objGlAccountId1) {
        objGlAccountId = objGlAccountId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.GlAccount", function () {
      it("get 'Gl Accounts' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Gl Accounts"];
