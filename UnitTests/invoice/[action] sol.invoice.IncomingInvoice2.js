...
describe("[action] sol.invoice.IncomingInvoice2", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.IncomingInvoice2", null, null).then(function success(objTempId) {
        interval = 4000;
        done();
      }, function error(err) {
        fail(err);
  ...
  describe("prepare incoming invoice", function () {
    ...
    it("get intray document", function (done) {
      expect(function () {
        test.Utils.getSord("ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/invoice [unit tests]/Resources/Scan_Invoice_Unittest").then(function success(intrayDocumentSord1) {
  ...
  describe("test incoming invoice -> 'Forward for approval' -> 'Approve Cost Object' -> 'Forward to clearing' -> 'Back to approval' -> 'Forward to ERP'", function () {
      ...
      expect(function () {
        ...
        test.Utils.changeMask(intrayDocumentSord, dstMaskId).then(function success(intrayDocumentSord1) {
          ...
          test.Utils.checkinSord(intrayDocumentSord1).then(function success1(checkinSordResult) {
            sourceId = "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/invoice [unit tests]/Resources/Scan_Invoice_Unittest";
    ...
    it("set invoice date, invoice number, purchase user, vendor", function (done) {
      expect(function () {
        test.Utils.getSord(objIncomingInvoiceId).then(function success(IncomingInvoiceSord) {
    ...
    it("'Forward to clearing' forwarding Subworkflow 'sol.invoice.Base.Accounting'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(subWfFlowId).then(function success(workflow) {
          succNodes = test.Utils.getSuccessorNodes(workflow, userNodeId, null, "sol.invoice.wf.node.sendToClearingCenterFromBooking");
          succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
    ...
    it("'Back to approval' forwarding Subworkflow 'sol.invoice.Base.Clearing'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(subWfFlowId).then(function success(workflow) {
          succNodes = test.Utils.getSuccessorNodes(workflow, userNodeId, null, "sol.invoice.wf.node.backToApproval");
          succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
    ...
    it("get active node 'Posting data export' (id = 3) of Subworkflow 'sol.invoice.Base.Export'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(invoiceFlowId).then(function success(workflow) {
          subWfs = [];
          subWorkflows = workflow.subWorkflows;
          ...
          for (i = 0; i < subWfs.length; i++) {
            if (subWfs[i].templateName == "sol.invoice.Base.Export") {
              subWf = subWfs[i];
              subWfFlowId = subWf.id;
              nodes = test.Utils.getActiveUserNodes(subWf);
              if (nodes.length > 0) {
    ...
    it("'Booking data response' forwarding Subworkflow 'sol.invoice.Base.Export'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(subWfFlowId).then(function success(workflow) {
          succNodes = test.Utils.getSuccessorNodes(workflow, userNodeId, null, "Booking data response");
          succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
          test.Utils.forwardWorkflowTask(subWfFlowId, userNodeId, succNodesIds, "Unittest 'Booking data response'", true).then(function success1(forwardWorkflowTaskResult) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
