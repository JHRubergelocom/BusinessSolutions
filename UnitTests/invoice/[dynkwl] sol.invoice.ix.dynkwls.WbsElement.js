...
describe("[dynkwl] sol.invoice.ix.dynkwls.WbsElement", function () {
  var originalTimeout, objWbsElementId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("WbsElement").then(function success(objWbsElementId1) {
        objWbsElementId = objWbsElementId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.WbsElement", function () {
      it("get 'WBS elements' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["WBS elements"];
