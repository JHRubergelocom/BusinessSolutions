...
describe("[action] sol.invoice.ZugferdIncomingInvoice3", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.ZugferdIncomingInvoice3", null, null).then(function success(objTempId) {
        interval = 4000;
        done();
      }, function error(err) {
        fail(err);
  ...
  describe("test incoming invoice -> 'Forward for approval' -> 'Back to previous user' -> 'Approve Cost Object' -> 'Forward to ERP'", function () {
    ...
    it("'Back to previous user' forwarding Subworkflow 'sol.invoice.Base.Approval'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(subWfFlowId).then(function success(workflow) {
          succNodes = test.Utils.getSuccessorNodes(workflow, userNodeId, null, "sol.invoice.wf.node.backtoprevioususer");
          succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
