...
describe("[action] sol.invoice.ZugferdIncomingInvoice3", function () {
  ...
  describe("test incoming invoice -> 'Forward for approval' -> 'Back to previous user' -> 'Approve Cost Object' -> 'Forward to ERP'", function () {
    ...
    it("'Back to previous user' forwarding Subworkflow 'sol.invoice.Base.ApprovalLines'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(subWfFlowId).then(function success(workflow) {
    ...
    it("get active node 'Cost object approval' (id = 34) of Subworkflow 'sol.invoice.Base.ApprovalLines'", function (done) {
      expect(function () {
        test.Utils.getWorkflow(invoiceFlowId).then(function success(workflow) {
          subWfs = [];
          subWorkflows = workflow.subWorkflows;
