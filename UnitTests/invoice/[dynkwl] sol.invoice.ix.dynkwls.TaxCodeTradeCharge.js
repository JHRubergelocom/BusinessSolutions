...
describe("[dynkwl] sol.invoice.ix.dynkwls.TaxCodeTradeCharge", function () {
  var originalTimeout, objTaxCodeTradeChargeId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("TaxCodeTradeCharge").then(function success(objTaxCodeTradeChargeId1) {
        objTaxCodeTradeChargeId = objTaxCodeTradeChargeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.TaxCodeTradeCharge", function () {
      it("get 'Tax codes trade charges' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Tax codes trade charges"];
            kwlConfig.searchParams = ["1000", "DE", "C", "C"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
