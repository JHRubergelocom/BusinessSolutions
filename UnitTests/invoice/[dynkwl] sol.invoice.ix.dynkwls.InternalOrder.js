...
describe("[dynkwl] sol.invoice.ix.dynkwls.InternalOrder", function () {
  var originalTimeout, objInternalOrderId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("InternalOrder").then(function success(objInternalOrderId1) {
        objInternalOrderId = objInternalOrderId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.InternalOrder", function () {
      it("get 'Internal orders' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Internal orders"];
