...
describe("[dynkwl] sol.invoice.ix.dynkwls.PurchaseOrderNo", function () {
  var originalTimeout, objPurchaseOrderNoId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PurchaseOrderNo").then(function success(objPurchaseOrderNoId1) {
        objPurchaseOrderNoId = objPurchaseOrderNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.PurchaseOrderNo", function () {
      it("get 'Purchase order no' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Purchase order no"];
