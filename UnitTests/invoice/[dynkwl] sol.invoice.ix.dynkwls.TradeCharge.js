...
describe("[dynkwl] sol.invoice.ix.dynkwls.TradeCharge", function () {
  var originalTimeout, objTradeChargeId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("TradeCharge").then(function success(objTradeChargeId1) {
        objTradeChargeId = objTradeChargeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.invoice.ix.dynkwl.TradeCharge", function () {
      it("get 'Trade charges' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Trade charges"];
