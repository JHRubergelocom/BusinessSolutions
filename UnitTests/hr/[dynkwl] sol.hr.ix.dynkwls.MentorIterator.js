...
describe("[dynkwl] sol.hr.ix.dynkwls.MentorIterator", function () {
  var originalTimeout, objMentorIteratorId, config;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("MentorIterator").then(function success(objMentorIteratorId1) {
        objMentorIteratorId = objMentorIteratorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.dynkwl.MentorIterator", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.dynkwl.MentorIterator",
            classConfig: {},
            method: "initialize",
            params: [config]
    ...
    describe("sol.hr.ix.dynkwl.MentorIterator", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objMentorIteratorId,
            dynKwl: "sol.hr.ix.dynkwl.MentorIterator"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
