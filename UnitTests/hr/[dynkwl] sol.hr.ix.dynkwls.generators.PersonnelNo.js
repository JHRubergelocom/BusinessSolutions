...
describe("[dynkwl] sol.hr.ix.dynkwls.generators.PersonnelNo", function () {
  var originalTimeout, objGeneratorsPersonnelNoId, config;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("GeneratorsPersonnelNo").then(function success(objGeneratorsPersonnelNoId1) {
        objGeneratorsPersonnelNoId = objGeneratorsPersonnelNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.dynkwl.generators.PersonnelNo", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.dynkwl.generators.PersonnelNo",
            classConfig: {},
            method: "initialize",
            params: [config]
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.hr.ix.dynkwl.generators.PersonnelNo", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objGeneratorsPersonnelNoId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objGeneratorsPersonnelNoId,
            dynKwl: "sol.hr.ix.dynkwl.generators.PersonnelNo"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
