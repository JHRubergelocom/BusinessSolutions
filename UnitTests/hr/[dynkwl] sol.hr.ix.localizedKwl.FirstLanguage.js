...
describe("[dynkwl] sol.hr.ix.localizedKwl.FirstLanguage", function () {
  var originalTimeout, objFirstLanguageId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("FirstLanguage").then(function success(objFirstLanguageId1) {
        objFirstLanguageId = objFirstLanguageId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.localizedKwl.FirstLanguage", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.localizedKwl.FirstLanguage",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.hr.ix.localizedKwl.FirstLanguage", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objFirstLanguageId,
            dynKwl: "sol.hr.ix.localizedKwl.FirstLanguage"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
