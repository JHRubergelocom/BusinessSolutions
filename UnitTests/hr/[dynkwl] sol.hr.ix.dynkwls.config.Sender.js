...
describe("[dynkwl] sol.hr.ix.dynkwls.config.Sender", function () {
  var originalTimeout, objConfigSenderId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("objConfigSenderId").then(function success(objConfigSenderId1) {
        objConfigSenderId = objConfigSenderId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.hr.ix.dynkwl.config.Sender", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objConfigSenderId,
            dynKwl: "sol.hr.ix.dynkwl.config.Sender"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
