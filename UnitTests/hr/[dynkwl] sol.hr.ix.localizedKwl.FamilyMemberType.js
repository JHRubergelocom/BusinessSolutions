...
describe("[dynkwl] sol.hr.ix.localizedKwl.FamilyMemberType", function () {
  var originalTimeout, objFamilyMemberTypeId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("FamilyMemberType").then(function success(objFamilyMemberTypeId1) {
        objFamilyMemberTypeId = objFamilyMemberTypeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.localizedKwl.FamilyMemberType", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.localizedKwl.FamilyMemberType",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.hr.ix.localizedKwl.FamilyMemberType", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objFamilyMemberTypeId,
            dynKwl: "sol.hr.ix.localizedKwl.FamilyMemberType"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
