...
describe("[dynkwl] sol.hr.ix.dynkwls.AuthorityIterator", function () {
  var originalTimeout, objAuthorityIteratorId, config;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("AuthorityIterator").then(function success(objAuthorityIteratorId1) {
        objAuthorityIteratorId = objAuthorityIteratorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.dynkwl.AuthorityIterator", function () {
      it("initialize", function (done) {
        expect(function () {
          config = {};
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.dynkwl.AuthorityIterator",
            classConfig: {},
            method: "initialize",
            params: [config]
    ...
    describe("sol.hr.ix.dynkwl.AuthorityIterator", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objAuthorityIteratorId,
            dynKwl: "sol.hr.ix.dynkwl.AuthorityIterator"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
