...
describe("[dynkwl] sol.hr.ix.localizedKwl.WorkscheduleRegulation", function () {
  var originalTimeout, objWorkscheduleRegulationId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("WorkscheduleRegulation").then(function success(objWorkscheduleRegulationId1) {
        objWorkscheduleRegulationId = objWorkscheduleRegulationId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.localizedKwl.WorkscheduleRegulation", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.localizedKwl.WorkscheduleRegulation",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.hr.ix.localizedKwl.WorkscheduleRegulation", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objWorkscheduleRegulationId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objWorkscheduleRegulationId,
            dynKwl: "sol.hr.ix.localizedKwl.WorkscheduleRegulation"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
