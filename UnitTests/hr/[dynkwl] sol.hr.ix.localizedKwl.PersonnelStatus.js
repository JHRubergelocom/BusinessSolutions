...
describe("[dynkwl] sol.hr.ix.localizedKwl.PersonnelStatus", function () {
  var originalTimeout, objPersonnelStatusId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PersonnelStatus").then(function success(objPersonnelStatusId1) {
        objPersonnelStatusId = objPersonnelStatusId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.hr.ix.dynkwl.PersonnelStatus", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objPersonnelStatusId,
            dynKwl: "sol.hr.ix.localizedKwl.PersonnelStatus"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
