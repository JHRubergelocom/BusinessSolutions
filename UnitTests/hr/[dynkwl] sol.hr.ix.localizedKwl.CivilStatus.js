...
describe("[dynkwl] sol.hr.ix.localizedKwl.CivilStatus", function () {
  var originalTimeout, objCivilStatusId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CivilStatus").then(function success(objCivilStatusId1) {
        objCivilStatusId = objCivilStatusId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.localizedKwl.CivilStatus", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.localizedKwl.CivilStatus",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.hr.ix.localizedKwl.CivilStatus", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objCivilStatusId,
            dynKwl: "sol.hr.ix.localizedKwl.CivilStatus"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
