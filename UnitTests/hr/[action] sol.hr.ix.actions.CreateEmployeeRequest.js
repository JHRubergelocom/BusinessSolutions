...
describe("[action] sol.hr.ix.actions.CreateEmployeeRequest", function () {
  var personnelFileTypes, objIdHr, objIdRe,
      ...
      requestTypes, originalTimeout;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Actions.CreateEmployeeRequest", null, null).then(function success(objTempId1) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
      ...
      describe("'OK' -> 'Reject'", function () {
        ...
        describe("test finish createemployeerequest", function () {
          ...
          it("'Reject' forwarding Workflow", function (done) {
            expect(function () {
              test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
                succNodes = test.Utils.getSuccessorNodes(workflow, userNodeId, null, "sol.hr.personnel.node.employeerequest.reject");
                succNodesIds = test.Utils.getSuccessorNodesIds(succNodes);
          ...
          it("fill personnelfile sord", function (done) {
            expect(function () {
              test.Utils.getSord(wfInfo.objId).then(function success(sordHr3) {
                objIdHr = wfInfo.objId;
                test.Utils.updateKeywording(sordHr3, { HR_PERSONNEL_FIRSTNAME: "Nobody", HR_PERSONNEL_LASTNAME: "Perfect", HR_PERSONNEL_ELOUSERID: "Administrator" }, true).then(function success1(updateKeywordingResult) {
                  done();
                }, function error(err) {
                  fail(err);
                  console.error(err);
