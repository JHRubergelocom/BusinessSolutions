...
describe("[action] sol.hr.ix.actions.CreateFile", function () {
  var originalTimeout, personnelFileTypes,
      configAction, wfInfo, succNodes, succNodesIds, objIdHr;
      ...
      it("fill personnel file sord", function (done) {
        expect(function () {
          test.Utils.getSord(wfInfo.objId).then(function success(sordHr) {
            ...
            test.Utils.updateKeywording(sordHr, { HR_PERSONNEL_FIRSTNAME: "Bernd", HR_PERSONNEL_LASTNAME: "Stromberg" }, true).then(function success1(updateKeywordingResult) {
  ...
  describe("test cancel createfile", function () {
    ...
    it("fill personnel file sord", function (done) {
      expect(function () {
        test.Utils.getSord(wfInfo.objId).then(function success(sordHr) {
          test.Utils.updateKeywording(sordHr, { HR_PERSONNEL_FIRSTNAME: "Nils", HR_PERSONNEL_LASTNAME: "Armstrong" }, true).then(function success1(updateKeywordingResult) {
            test.Utils.updateSord(sordHr, [{ key: "desc", value: "Unittest desc" }]).then(function success2(updateSordResult) {
              done();
            }, function error(err) {
              fail(err);
