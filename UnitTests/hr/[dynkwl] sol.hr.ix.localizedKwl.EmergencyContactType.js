...
describe("[dynkwl] sol.hr.ix.localizedKwl.EmergencyContactType", function () {
  var originalTimeout, objEmergencyContactTypeId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("EmergencyContactType").then(function success(objEmergencyContactTypeId1) {
        objEmergencyContactTypeId = objEmergencyContactTypeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.hr.ix.localizedKwl.EmergencyContactType", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteLib1", {
            className: "sol.hr.ix.localizedKwl.EmergencyContactType",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.hr.ix.localizedKwl.EmergencyContactType", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objEmergencyContactTypeId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_hr_service_ExecuteDynKwl", {
            objId: objEmergencyContactTypeId,
            dynKwl: "sol.hr.ix.localizedKwl.EmergencyContactType"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
