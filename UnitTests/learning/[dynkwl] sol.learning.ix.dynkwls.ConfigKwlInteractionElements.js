...
describe("[dynkwl] sol.learning.ix.dynkwls.InteractionElements", function () {
  var originalTimeout, objInteractionElementsId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("InteractionElements").then(function success(objInteractionElementsId1) {
        objInteractionElementsId = objInteractionElementsId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.configKwl.InteractionElements", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl1", {
            objId: objInteractionElementsId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl1", {
            objId: objInteractionElementsId,
            dynKwl: "sol.learning.ix.configKwl.InteractionElements"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
