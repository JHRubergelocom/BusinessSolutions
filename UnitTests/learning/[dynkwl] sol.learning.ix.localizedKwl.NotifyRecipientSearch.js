...
describe("[dynkwl] sol.learning.ix.localizedKwl.NotifyRecipientSearch", function () {
  var originalTimeout, objNotifyRecipientSearchId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("NotifyRecipientSearch").then(function success(objNotifyRecipientSearchId1) {
        objNotifyRecipientSearchId = objNotifyRecipientSearchId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.NotifyRecipientSearch", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl1", {
            objId: objNotifyRecipientSearchId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl1", {
            objId: objNotifyRecipientSearchId,
            dynKwl: "sol.learning.ix.localizedKwl.NotifyRecipientSearch"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
