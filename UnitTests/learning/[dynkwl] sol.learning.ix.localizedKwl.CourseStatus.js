...
describe("[dynkwl] sol.learning.ix.localizedKwl.CourseStatus", function () {
  var originalTimeout, objCourseStatusId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CourseStatus").then(function success(objCourseStatusId1) {
        objCourseStatusId = objCourseStatusId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.CourseStatus", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objCourseStatusId,
            dynKwl: "sol.learning.ix.localizedKwl.CourseStatus"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
