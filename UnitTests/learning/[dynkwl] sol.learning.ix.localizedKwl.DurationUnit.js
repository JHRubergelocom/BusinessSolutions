...
describe("[dynkwl] sol.learning.ix.localizedKwl.DurationUnit", function () {
  var originalTimeout, objDurationUnitId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("DurationUnit").then(function success(objDurationUnitId1) {
        objDurationUnitId = objDurationUnitId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.DurationUnit", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objDurationUnitId,
            dynKwl: "sol.learning.ix.localizedKwl.DurationUnit"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
