...
describe("[dynkwl] sol.learning.ix.dynkwls.generators.CourseNo", function () {
  var originalTimeout, objGeneratorsCourseNoId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("GeneratorsCourseNo").then(function success(objGeneratorsCourseNoId1) {
        objGeneratorsCourseNoId = objGeneratorsCourseNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.generators.CourseNo", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objGeneratorsCourseNoId,
            dynKwl: "sol.learning.ix.dynkwl.generators.CourseNo"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
