...
describe("[dynkwl] sol.learning.ix.localizedKwl.CourseDifficulty", function () {
  var originalTimeout, objCourseDifficultyId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CourseDifficulty").then(function success(objCourseDifficultyId1) {
        objCourseDifficultyId = objCourseDifficultyId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.CourseDifficulty", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objCourseDifficultyId,
            dynKwl: "sol.learning.ix.localizedKwl.CourseDifficulty"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
