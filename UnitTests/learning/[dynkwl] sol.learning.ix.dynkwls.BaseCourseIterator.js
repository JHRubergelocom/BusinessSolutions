...
describe("[dynkwl] sol.learning.ix.dynkwls.BaseCourseIterator", function () {
  var originalTimeout, objBaseCourseIteratorId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("BaseCourseIterator").then(function success(objBaseCourseIteratorId1) {
        objBaseCourseIteratorId = objBaseCourseIteratorId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.BaseCourseIterator", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objBaseCourseIteratorId,
            dynKwl: "sol.learning.ix.dynkwl.BaseCourseIterator"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
