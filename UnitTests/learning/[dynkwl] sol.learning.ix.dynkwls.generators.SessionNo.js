...
describe("[dynkwl] sol.learning.ix.dynkwls.generators.SessionNo", function () {
  var originalTimeout, objGeneratorsSessionNoId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("GeneratorsSessionNo").then(function success(objGeneratorsSessionNoId1) {
        objGeneratorsSessionNoId = objGeneratorsSessionNoId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.generators.SessionNo", function () {
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objGeneratorsSessionNoId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl", {
            objId: objGeneratorsSessionNoId,
            dynKwl: "sol.learning.ix.dynkwl.generators.SessionNo"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
