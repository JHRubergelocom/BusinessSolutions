...
describe("[dynkwl] sol.learning.ix.dynkwls.NotifyTemplate", function () {
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.learning.ix.dynkwl.NotifyTemplate", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_learning_service_ExecuteDynKwl1", {
            objId: objNotifyTemplateId,
            dynKwl: "sol.learning.ix.dynkwl.NotifyTemplate"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
