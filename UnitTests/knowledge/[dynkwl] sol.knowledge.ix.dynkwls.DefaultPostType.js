...
describe("[dynkwl] sol.knowledge.ix.dynkwls.DefaultPostType", function () {
  var originalTimeout, objDefaultPostTypeId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("DefaultPostType").then(function success(objDefaultPostTypeId1) {
        objDefaultPostTypeId = objDefaultPostTypeId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.knowledge.ix.dynkwl.DefaultPostType", function () {
      it("get 'Default Post Type' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            objId: "ARCPATH[(E10E1000-E100-E100-E100-E10E10E10E00)]:/Business Solutions/knowledge/Configuration/kwl.config"
          }).then(function success(configResult) {
            kwlConfig = configResult.config["Default Post Type"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
