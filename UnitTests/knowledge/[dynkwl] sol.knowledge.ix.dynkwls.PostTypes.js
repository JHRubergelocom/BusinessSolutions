...
describe("[dynkwl] sol.knowledge.ix.dynkwls.PostTypes", function () {
  var originalTimeout, objPostTypesId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("PostTypes").then(function success(objPostTypesId1) {
        objPostTypesId = objPostTypesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.knowledge.ix.dynkwl.PostTypes", function () {
      it("get 'Post Type Templates' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Post Type Templates"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
