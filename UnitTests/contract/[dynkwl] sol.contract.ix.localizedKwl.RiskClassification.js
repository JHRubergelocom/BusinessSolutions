...
describe("[dynkwl] sol.contract.ix.localizedKwl.RiskClassification", function () {
  var originalTimeout, objRiskClassificationId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("RiskClassification").then(function success(objRiskClassificationId1) {
        objRiskClassificationId = objRiskClassificationId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.RiskClassification", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteDynKwl", {
            objId: objRiskClassificationId,
            dynKwl: "sol.contract.ix.localizedKwl.RiskClassification"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
