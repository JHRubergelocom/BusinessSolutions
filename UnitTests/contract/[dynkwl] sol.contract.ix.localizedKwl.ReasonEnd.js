...
describe("[dynkwl] sol.contract.ix.localizedKwl.ReasonEnd", function () {
  var originalTimeout, objReasonEndId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ReasonEnd").then(function success(objReasonEndId1) {
        objReasonEndId = objReasonEndId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.ReasonEnd", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteDynKwl", {
            objId: objReasonEndId,
            dynKwl: "sol.contract.ix.localizedKwl.ReasonEnd"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
