...
describe("[action] sol.contract.ix.actions.ContractConcluded", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.ContractConcluded", null, null).then(function success(objTempId) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("test finish concludecontract", function () {
    it("should throw if executed without 'user' and 'objId'", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_contract_action_ConcludeContract", {
    ...
    it("start action conclude workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdCr1
        };
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_contract_action_ConcludeContract", configAction, []).then(function success(jsonResults) {
          test.Utils.handleAllEvents(jsonResults).then(function success1(wfInfo1) {
            wfInfo = wfInfo1;
            done();
          }, function error(err) {
    ...
    it("finish concludecontract forwarding workflow", function (done) {
      expect(function () {
        test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
  ...
  describe("test cancel concludecontract", function () {
    it("start action conclude workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdCr2
    ...
    it("cancel concludecontract forwarding workflow", function (done) {
      expect(function () {
        test.Utils.getWorkflow(wfInfo.flowId).then(function success(workflow) {
