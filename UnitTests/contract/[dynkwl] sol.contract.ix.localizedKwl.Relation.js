...
describe("[dynkwl] sol.contract.ix.localizedKwl.Relation", function () {
  var originalTimeout, objRelationId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Relation").then(function success(objRelationId1) {
        objRelationId = objRelationId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.Relation", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteDynKwl", {
            objId: objRelationId,
            dynKwl: "sol.contract.ix.localizedKwl.Relation"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
