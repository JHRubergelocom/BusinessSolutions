...
describe("[dynkwl] sol.contract.ix.dynkwls.generators.ClauseId", function () {
  var originalTimeout, objgeneratorsClauseIdId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsClauseId").then(function success(objgeneratorsClauseIdId1) {
        objgeneratorsClauseIdId = objgeneratorsClauseIdId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.generators.ClauseId", function () {
      it("get 'Clause ID generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Clause ID generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
