...
describe("[dynkwl] sol.contract.ix.dynkwls.ContractPartnerContactAdditional", function () {
  var originalTimeout, objContractPartnerContactAdditionalId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ContractPartnerContactAdditional").then(function success(objContractPartnerContactAdditionalId1) {
        objContractPartnerContactAdditionalId = objContractPartnerContactAdditionalId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.ContractPartnerContactAdditional", function () {
      it("get 'Additional contract partner contacts' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Additional contract partner contacts"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteDynKwl", {
            objId: objContractPartnerContactAdditionalId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
