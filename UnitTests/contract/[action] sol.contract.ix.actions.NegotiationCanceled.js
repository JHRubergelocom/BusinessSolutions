...
describe("[action] sol.contract.ix.actions.NegotiationCanceled", function () {
  var objIdCr, contractTypes, configAction,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Actions.NegotiationCanceled", null, null).then(function success(objTempId1) {
  ...
  describe("Test Lib Functions", function () {
    describe("sol.contract.ix.actions.CancelNegotiation", function () {
      it("getName", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteLib1", {
            className: "sol.contract.ix.actions.CancelNegotiation",
    ...
    it("fill contract sord", function (done) {
      expect(function () {
        test.Utils.getSord(wfInfo.objId).then(function success(sordCr) {
          ...
          test.Utils.updateKeywording(sordCr, { CONTRACT_NAME: "Unittest CONTRACT_NAME1", CONTACT_FIRSTNAME: "Bernd", CONTACT_LASTNAME: "Stromberg" }, true).then(function success1(updateKeywordingResult) {
  ...
  describe("test finish cancel negotiation", function () {
    it("should throw if executed without 'user' and 'objId'", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_contract_action_CancelNegotiation", {
        }).then(function success(jsonResult) {
          fail(jsonResult);
          done();
        }, function error(err) {
    ...
    it("start action negotiationCanceled workflow", function (done) {
      expect(function () {
        configAction = {
          objId: objIdCr
        };
        wfInfo = {};
        test.Utils.executeIxActionHandler("RF_sol_contract_action_CancelNegotiation", configAction, []).then(function success(jsonResults) {
          test.Utils.handleAllEvents(jsonResults).then(function success1(wfInfo1) {
            wfInfo = wfInfo1;
            done();
          }, function error(err) {
    ...
    it("remove workflows", function (done) {
      expect(function () {
        test.Utils.getFinishedWorkflows(objIdCr).then(function success(wfs) {
          test.Utils.removeFinishedWorkflows(wfs).then(function success1(removeFinishedWorkflowsResult) {
            done();
          }, function error(err) {
            fail(err);
