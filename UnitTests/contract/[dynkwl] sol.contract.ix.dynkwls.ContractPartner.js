...
describe("[dynkwl] sol.contract.ix.dynkwls.ContractPartner", function () {
  var originalTimeout, objContractPartnerId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ContractPartner").then(function success(objContractPartnerId1) {
        objContractPartnerId = objContractPartnerId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.ContractPartner", function () {
      it("get 'Contract partners' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Contract partners"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
