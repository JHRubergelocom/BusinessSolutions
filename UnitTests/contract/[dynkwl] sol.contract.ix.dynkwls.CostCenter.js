...
describe("[dynkwl] sol.contract.ix.dynkwls.CostCenter", function () {
  var originalTimeout, objCostCenterId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CostCenter").then(function success(objCostCenterId1) {
        objCostCenterId = objCostCenterId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.CostCenter", function () {
      it("get 'Cost centers' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Cost centers"];
