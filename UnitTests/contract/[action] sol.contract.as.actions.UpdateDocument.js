...
describe("[action] sol.contract.as.actions.UpdateDocument", function () {
  var objIdCr, i, contractTypes,
      succNodes, succNodesIds, objIdReport,
      params, targetId, templateId, templateFolderId,
      wfInfo, configAction, dvReport, originalTimeout;
    ...
    it("fill contract sord", function (done) {
      expect(function () {
        test.Utils.getSord(wfInfo.objId).then(function success(sordCr) {
          ...
          test.Utils.updateKeywording(sordCr, { CONTRACT_NAME: "Unittest CONTRACT_NAME1", CONTACT_FIRSTNAME: "Bernd", CONTACT_LASTNAME: "Stromberg", PARTNER_NAME: "Bernd das Brot" }, true).then(function success1(updateKeywordingResult) {
  ...
  describe("test update document", function () {
    it("start as function update document", function (done) {
      expect(function () {
        test.Utils.execute("RF_sol_common_service_ExecuteAsAction", {
          solution: "contract",
          action: "sol.contract.as.actions.CreateDocument",
          config: { objId: objIdReport, templateId: objIdReport }
        }, function (data) {
           // process result
        }, function (err) {
           // error handling
    ...
    it("report must be available", function (done) {
      test.Utils.getSord(objIdReport).then(function success(sordReport) {
        objIdReport = objIdReport;
        expect(sordReport).toBeDefined();
        done();
      }, function error(err) {
        fail(err);
    ...
    it("doc.size report must be available", function (done) {
      test.Utils.getCurrentWorkVersion(objIdReport).then(function success(dvReport1) {
        dvReport = dvReport1;
        expect(dvReport.size).toBeDefined();
        done();
      }, function error(err) {
