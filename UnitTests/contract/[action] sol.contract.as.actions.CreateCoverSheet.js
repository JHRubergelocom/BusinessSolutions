...
describe("[action] sol.contract.as.actions.CreateCoverSheet", function () {
  var objIdCr, contractTypes, succNodes, succNodesIds,
      params, targetId, templateId,
      configAction, coversheetTypes, dvReport,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("As.Actions.CreateCoverSheet", null, null).then(function success(objTempId) {
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("test create coversheet from template", function () {
    it("coversheetTypes must be available", function (done) {
      test.Utils.execute("RF_sol_contract_service_PrepareContractCoverSheet", { targetId: objIdCr }).then(function success(prepareResult) {
        ...
        coversheetTypes = prepareResult.types;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
    ...
    it("coversheetTypes.length must greater than zero", function () {
      expect(coversheetTypes.length).toBeGreaterThan(0);
    });
    it("start as action create coversheet", function (done) {
      expect(function () {
        targetId = objIdCr;
        templateId = coversheetTypes[0].objId;
        params = { templateId: templateId, targetId: targetId };
        test.Utils.executeASActionHandler("contract", "sol.contract.as.actions.CreateContractCoverSheet", params, []).then(function success(jsonResults) {
          test.Utils.handleAllEvents(jsonResults).then(function success1(wfInfo1) {
            wfInfo = wfInfo1;
            done();
          }, function error(err) {
