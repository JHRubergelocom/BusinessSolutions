...
describe("[dynkwl] sol.contract.ix.dynkwls.ContactList", function () {
  var originalTimeout, objContactListId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("ContactList").then(function success(objContactListId1) {
        objContactListId = objContactListId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.ContactList", function () {
      it("get 'Contact lists' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Contact lists"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
