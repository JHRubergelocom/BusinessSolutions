...
describe("[dynkwl] sol.contract.ix.dynkwls.Company", function () {
  var originalTimeout, objCompanyId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("Company").then(function success(objCompanyId1) {
        objCompanyId = objCompanyId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.Company", function () {
      it("get 'Companies' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config.Companies;
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
