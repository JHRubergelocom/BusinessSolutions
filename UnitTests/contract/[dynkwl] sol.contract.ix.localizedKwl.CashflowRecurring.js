...
describe("[dynkwl] sol.contract.ix.localizedKwl.CashflowRecurring", function () {
  var originalTimeout, objCashflowRecurringId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CashflowRecurring").then(function success(objCashflowRecurringId1) {
        objCashflowRecurringId = objCashflowRecurringId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.contract.ix.localizedKwl.CashflowRecurring", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteLib1", {
            className: "sol.contract.ix.localizedKwl.CashflowRecurring",
            classConfig: {},
            method: "initialize",
            params: []
    ...
    describe("sol.contract.ix.localizedKwl.CashflowRecurring", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_contract_service_ExecuteDynKwl", {
            objId: objCashflowRecurringId,
            dynKwl: "sol.contract.ix.localizedKwl.CashflowRecurring"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
