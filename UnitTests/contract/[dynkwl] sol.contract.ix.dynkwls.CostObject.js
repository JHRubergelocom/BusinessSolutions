...
describe("[dynkwl] sol.contract.ix.dynkwls.CostObject", function () {
  var originalTimeout, objCostObjectId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CostObject").then(function success(objCostObjectId1) {
        objCostObjectId = objCostObjectId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.contract.ix.dynkwl.CostObject", function () {
      it("get 'Cost objects' configuration from kwl.config", function (done) {
        expect(function () {
