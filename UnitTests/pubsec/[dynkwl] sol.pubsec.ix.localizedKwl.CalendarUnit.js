...
describe("[dynkwl] sol.pubsec.ix.localizedKwl.CalendarUnit", function () {
  var originalTimeout, objCalendarUnitId;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("CalendarUnit").then(function success(objCalendarUnitId1) {
        objCalendarUnitId = objCalendarUnitId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Test Lib Functions", function () {
    describe("sol.pubsec.ix.localizedKwl.CalendarUnit", function () {
      it("initialize", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteLib1", {
            className: "sol.pubsec.ix.localizedKwl.CalendarUnit",
            classConfig: {},
            method: "initialize",
            params: []
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.pubsec.ix.dynkwl.CalendarUnit", function () {
      ...
      it("should throw if executed without 'providerConfig'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteDynKwl", {
            objId: objCalendarUnitId,
            dynKwl: "sol.pubsec.ix.localizedKwl.CalendarUnit"
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
