...
describe("[action] sol.pubsec.as.actions.CreateFileCoverSheet", function () {
  var originalTimeout, objTempId, fileTypes, objFPId,
      configAction, checkResult,
      wfInfo, succNodes, succNodesIds,
      filecoversheetTypes, repTemplateId, parentId, params, targetId,
      evInfo, dvReport;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("As.Actions.CreateFileCoverSheet", null, null).then(function success(objTempId1) {
  ...
  describe("test create filecoversheet", function () {
    ...
    describe("test create filecoversheet", function () {
      it("check preconditions should not throw", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_pubsec_service_CheckFileCoverSheetPreconditions", { targetId: objTempId }).then(function success(checkResult1) {
            checkResult = checkResult1;
            done();
          }, function error(err) {
            fail(err);
      ...
      it("filecoversheetTypes must be available", function (done) {
        test.Utils.execute("RF_sol_pubsec_service_GetFileCoverSheetTypes", {}).then(function success(filecoversheetTypes1) {
          filecoversheetTypes = filecoversheetTypes1;
          expect(filecoversheetTypes).toBeDefined();
          done();
        }, function error(err) {
          fail(err);
          console.error(err);
      ...
      it("filecoversheetTypes.length must greater than zero", function () {
        expect(filecoversheetTypes.length).toBeGreaterThan(0);
      });
      it("start as action create filecoversheet", function (done) {
        expect(function () {
          repTemplateId = filecoversheetTypes[0].objId;
          parentId = objTempId;
          targetId = objTempId;
          params = { templateId: repTemplateId, parentId: parentId, targetId: targetId };
          evInfo = {};
          test.Utils.executeASActionHandler("pubsec", "sol.pubsec.as.actions.CreateFileCoverSheet", params, []).then(function success(jsonResults) {
