...
describe("[service] sol.pubsec.ix.services.Open", function () {
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("Open").then(function success(objTempId) {
  ...
  describe("Test Lib Functions", function () {
    describe("sol.pubsec.ix.services.CheckOpenPreconditions", function () {
      it("checkFilePreconditions", function (done) {
        expect(function () {
          sord = { id: sordFile.id };
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteLib1", {
            className: "sol.pubsec.ix.services.CheckOpenPreconditions",
            classConfig: {},
  ...
  describe("Tests Registered Functions", function () {
    describe("RF_sol_pubsec_service_CheckOpenPreconditions", function () {
      it("should throw if executed without Parameter  'targetId'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_pubsec_service_CheckOpenPreconditions", {
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
      ...
      it("should not throw if executed with Parameter {'targetId': 1 }", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_pubsec_service_CheckOpenPreconditions", {
            targetId: 1
          }).then(function success(jsonResult) {
