...
describe("[action] sol.pubsec.as.actions.UpdateDocument", function () {
  var originalTimeout, objTempId, fileTypes, objFPId,
      configAction, checkResult, objReportId,
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("As.Actions.UpdateDocument", null, null).then(function success(objTempId1) {
  ...
  describe("test update document", function () {
    ...
    describe("test create document from template", function () {
      ...
      it("report must be available", function (done) {
        test.Utils.getSord(wfInfo.objId).then(function success(sordReport) {
          objReportId = wfInfo.objId;
          expect(objReportId).toBeDefined();
    ...
    describe("test update document", function () {
      it("start as action update document", function (done) {
        expect(function () {
          params = { objId: objReportId };
          wfInfo = {};
          test.Utils.executeASActionHandler("pubsec", "sol.pubsec.as.actions.CreateDocument", params, []).then(function success(jsonResults) {
            test.Utils.handleAllEvents(jsonResults).then(function success1(wfInfo1) {
              wfInfo = wfInfo1;
              done();
            }, function error(err) {
