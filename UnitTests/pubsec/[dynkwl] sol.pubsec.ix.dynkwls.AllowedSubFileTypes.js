...
describe("[dynkwl] sol.pubsec.ix.dynkwls.AllowedSubFileTypes", function () {
  var originalTimeout, objAllowedSubFileTypesId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("AllowedSubFileTypes").then(function success(objAllowedSubFileTypesId1) {
        objAllowedSubFileTypesId = objAllowedSubFileTypesId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.pubsec.ix.dynkwl.AllowedSubFileTypes", function () {
      it("get 'Allowed subfile types' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Allowed subfile types"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteDynKwl", {
            objId: objAllowedSubFileTypesId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
