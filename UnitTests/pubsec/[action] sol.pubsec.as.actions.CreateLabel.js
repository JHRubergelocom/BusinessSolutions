...
describe("[action] sol.pubsec.as.actions.CreateLabel", function () {
  var originalTimeout, objTempId, fileTypes, objFPId,
      configAction, checkResult,
      wfInfo, succNodes, succNodesIds,
      labelTypes, repTemplateId, parentId, params, targetId,
      evInfo, dvReport;
  ...
  beforeAll(function (done) {
    ...
    expect(function () {
      test.Utils.createTempSord("As.Actions.CreateLabel", null, null).then(function success(objTempId1) {
  ...
  describe("test create label", function () {
    ...
    describe("test create label", function () {
      it("check preconditions should not throw", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_pubsec_service_CheckLabelPreconditions", { targetId: objTempId }).then(function success(checkResult1) {
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("labelTypes must be available", function (done) {
        test.Utils.execute("RF_sol_pubsec_service_GetLabelTypes", {}).then(function success(labelTypes1) {
          labelTypes = labelTypes1;
      ...
      it("start as action create label", function (done) {
        expect(function () {
          ...
          evInfo = {};
          test.Utils.executeASActionHandler("pubsec", "sol.pubsec.as.actions.CreateLabel", params, []).then(function success(jsonResults) {
