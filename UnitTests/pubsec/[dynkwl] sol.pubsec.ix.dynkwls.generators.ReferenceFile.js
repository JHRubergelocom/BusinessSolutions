...
describe("[dynkwl] sol.pubsec.ix.dynkwls.generators.ReferenceFile", function () {
  var originalTimeout, objgeneratorsReferenceFileId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsReferenceFile").then(function success(objgeneratorsReferenceFileId1) {
        objgeneratorsReferenceFileId = objgeneratorsReferenceFileId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.pubsec.ix.dynkwl.generators.ReferenceFile", function () {
      it("get 'File reference generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["File reference generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteDynKwl", {
            objId: objgeneratorsReferenceFileId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
