...
describe("[dynkwl] sol.pubsec.ix.dynkwls.generators.ReferenceDocument", function () {
  var originalTimeout, objgeneratorsReferenceDocumentId, kwlConfig;

  beforeAll(function (done) {
    originalTimeout = jasmine.DEFAULT_TIMEOUT_INTERVAL;
    jasmine.DEFAULT_TIMEOUT_INTERVAL = 100000;
    expect(function () {
      test.Utils.createTempSord("generatorsReferenceDocument").then(function success(objgeneratorsReferenceDocumentId1) {
        objgeneratorsReferenceDocumentId = objgeneratorsReferenceDocumentId1;
        done();
      }, function error(err) {
        fail(err);
        console.error(err);
  ...
  describe("Tests Dynamic Keyword lists", function () {
    describe("sol.pubsec.ix.dynkwl.generators.ReferenceDocument", function () {
      it("get 'Document reference generators' configuration from kwl.config", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_common_service_GetConfig", {
            ...
            kwlConfig = configResult.config["Document reference generators"];
            done();
          }, function error(err) {
            fail(err);
            console.error(err);
      ...
      it("should throw if executed without 'dynkwl'", function (done) {
        expect(function () {
          test.Utils.execute("RF_sol_unittest_pubsec_service_ExecuteDynKwl", {
            objId: objgeneratorsReferenceDocumentId
          }).then(function success(jsonResult) {
            fail(jsonResult);
            done();
          }, function error(err) {
